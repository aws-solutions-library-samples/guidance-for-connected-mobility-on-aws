...
VEHICLES_TABLE = f"cms-{os.environ.get('DEPLOYMENT_STAGE', 'prod')}-storage-vehicles"
RECALLS_TABLE = f"cms-{os.environ.get('DEPLOYMENT_STAGE', 'prod')}-storage-recalls"
...
def main():
    ...
    # disappear from the UI (e.g. one make's recalls dominating when the
    # fleet's other-make matches sat below the cap). Keep all recalls — it's
    # still a bounded dataset (NHTSA typically returns <100 per make/model/year).
    ts_recalls = []
