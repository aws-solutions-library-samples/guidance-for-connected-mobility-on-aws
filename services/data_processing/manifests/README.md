...
### Match-Predicate Syntax

Match predicates disambiguate events with the same type-URL but different payloads. Two predicate kinds are supported as of v2.2.0:

#### Equality predicates (any key except `stringLabelEndsWith`)
...
#### `stringLabelEndsWith` predicate (v2.2.0; spec `2026-06-09-cms-oem1-dtc-engine-light-pipeline`)

```json
{
  "match": {
    "stringLabelEndsWith": ":custom:vha-diagnostics-processed-event"
  }
}
```

Matches when the decoded TriggeredEvent's `stringLabel` field ends with the given suffix string. Use for OEM1 custom events whose label includes a tenant UUID prefix (`aui:event:<tenantId>:custom:<name>`); suffix-matching avoids hardcoding the tenant identifier and is the correct predicate shape for multi-tenant OEM1 envelopes.

- **Schema enforces `minLength: 1`** to prevent the empty-string footgun (`"".endsWith("")` is universally true).
- **AND-semantics** when combined with equality predicates in the same `match` block.
- **Lenient default**: empty/missing `match` block → always match.
...
|---|---|---|
| v2.1.0 | 2026-06-08 | Initial OEM1 event_mappings; signal_mappings; derived_fields rule tables |
| v2.2.0 | 2026-06-10 | Added `match.stringLabelEndsWith` predicate (with `minLength: 1`) for tenant-prefixed string_label TriggeredEvents; added `vehicle_id_extraction.transform: "aui_asset_resolve"` for `aui:asset:<kind>/<UUID>` shard keys with vehicle/device kind dispatch (device kind delegates to OEM1 enrollment-table resolver, with DLQ fallback for unenrolled UUIDs). Spec: `2026-06-09-cms-oem1-dtc-engine-light-pipeline`.
...
| `cms.vha_diagnostic_event` | `stringLabelEndsWith: :custom:vha-diagnostics-processed-event` | indicator_state ∈ {ON, OFF} × DtcClear ∈ {present, absent} × dtc_code ∈ {non-empty, empty} | `occurred_at`, `indicator`, `indicator_state`, `indicator_extra_code`, `dtc_code`, `dtc_system`, `dtc_status`, `dtc_reporting_ecu`, `symptom_key`, `customer_action_key`, `action_text`, `category`, `severity_raw`, `symptom_text`, `cloud_arrival_time`, `vha_read_time`, `alert_trace_id`
...
### MaintenanceProcessor (canonical-DTC integration, 2026-06-09 / Path-ε)

Recognizes the canonical OEM1 DTC event:

- **`cms.vha_diagnostic_event`** (new, 2026-06-09) — single catch-all event type emitted from the `stringLabelEndsWith: :custom:vha-diagnostics-processed-event` matcher. The 4-state classification (Warning-with-DTC / Warning-no-DTC / Clear-Warning / Clear-DTC-indicator-active) is performed *inside* `MaintenanceProcessor.handleCanonicalIndicatorEvent` based on the `(indicator_state, dtc_clear, dtc_code)` triple — the manifest engine itself does not evaluate the state discriminator (per Fix Group 3.1 architectural decision; see review.md § Cycle 6). The handler dispatches to one of 4 outcomes:

| `indicator_state` | `dtc_clear` (tag) | `dtc_code` | `dtc-history.status` | Action queue (CRITICAL only) |
|---|---|---|---|---|
| `ON` | absent | non-empty | `ACTIVE` | yes |
| `ON` | absent | empty | `ACTIVE_NO_DTC` | yes |
| `OFF` | `Y` | any | `CLEARED` (Query+Update of matching active rows) | no |
| `ON` | `Y` | any | `DTC_CLEARED_INDICATOR_ACTIVE` (Query+Update of matching active rows) | no |
| any other | any | any | log + drop (defensive) | no |

Severity vocabulary mapping (vendor-supplied, authoritative): `URGENT → CRITICAL`, `HIGH → HIGH`, `MEDIUM → MEDIUM`, `LOW → LOW`, missing/unknown → `HIGH`. CRITICAL writes `cms-staging-vfo-action-queue` PENDING row.

**Source disambiguator**: `dtc-history` rows from this path use `source: "oem1-uds-dtc"` (parity with FWE's `fwe-uds-dtc`). Action-queue rows use `source: "dtc-critical"`, `sourceTag: "oem1-uds-dtc"`.

**Schema additions to `cms-staging-storage-dtc-history`** (additive, nullable; no DDL change since DDB is schemaless):
- `status` enum extended: pre-existing `ACTIVE` / `CLEARED` plus new `ACTIVE_NO_DTC` and `DTC_CLEARED_INDICATOR_ACTIVE`
- New columns: `indicator`, `indicator_extra_code`, `symptom_key`, `customer_action_key`, `category`, `cloud_arrival_time`, `vha_read_time`, `alert_trace_id`
- `description` populated from VHA `Action` tag; `agentResponse` populated from VHA `Symptom` tag
...
- Operator runbook: `docs/OEM1_DTC_PIPELINE.md`
