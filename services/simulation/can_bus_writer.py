...
def detect_interface() -> tuple:
    ...
    if system == 'Linux':
        ...
        print("⚠️  vcan0 not found. Run setup_vcan() or:")
...
def setup_vcan(interface: str = 'vcan0') -> bool:
    """Set up virtual CAN interface on Linux. Requires sudo."""
    ...
    try:
        ...
        if 'RTNETLINK answers: File exists' in (e.stderr or b'').decode():
            # Already exists, just bring it up
            subprocess.run(['sudo', 'ip', 'link', 'set', interface, 'up'], check=True)
