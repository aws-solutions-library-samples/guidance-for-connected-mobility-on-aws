...
def _start(config):
    ...
    if mode == "fwe":
        ...
        if isinstance(config.get("vehicles"), list) and config["vehicles"]:
            ...
            for target in targets:
                ...
                if has_campaign:
                    break
            if not has_campaign:
                # Warn (but do NOT abort) — surfaces the risk without
                # blocking the operator.
                campaign_warning = (
                    f"No active campaign assigned to {vin} — FWE agent "
                    "will start but may not transmit signals until a "
                    "campaign is assigned."
                )
                print(f"⚠️ CAMPAIGN-WARN: {campaign_warning}")
                config["_campaign_warning"] = campaign_warning
        ...
        if not existing_agent:
            ...
            # Existing agent — discover its vcan. If discovery fails, return
            # 500 with the diagnostic instead of silently routing to vcan0.
            try:
    ...
    # Non-blocking advisory when the FWE agent started without a
    # campaign assigned. Frontend may surface as info flash; absence of
    # this key means "campaign was found" and no advisory needed.
    campaign_warning = config.get("_campaign_warning")
    if campaign_warning:
        resp_body["warning"] = campaign_warning
