...
Usage:
    ...
    python3 setup_fwe.py --vehicle-id YOUR_VEHICLE_VIN --profile default

    # Run FWE container
    python3 setup_fwe.py --vehicle-id YOUR_VEHICLE_VIN --profile default --run
"""
...
def run_fwe(endpoint, thing_name, cert_file, key_file, topic_prefix):
    ...
    # Write input config with jobs/shadow topic prefixes (not exposed as CLI flags)
    import json as _json, tempfile
    input_cfg = _json.dumps({
        "version": "1.0",
        "networkInterfaces": [
            {"canInterface": {"interfaceName": "vcan0", "protocolName": "CAN", "protocolVersion": "2.0A"},
             "interfaceId": "1", "type": "canInterface"},
            {"obdInterface": {"interfaceName": "vcan0", "obdStandard": "J1979",
    ...
    cfg_path = os.path.join(OUTPUT_DIR, 'input-config.json')
    ...
    cmd = [
        ...
        '--env', 'COMMANDS_TOPIC_PREFIX=cms/commands/',
        '--env', 'CAN_BUS0=vcan0',
        '--env', 'LOG_LEVEL=Info',
        FWE_IMAGE,
