...
|-------|---------|---|
| `cms-{stage}-storage-fleets-*` | Fleet metadata | PK: `fleetId` → includes `data_source` (`'vehicle-telemetry' \| 'cloud-telemetry'`) + optional `transform_manifest_id` (off-board fleets only) + optional `decoder_manifest_id` (canonical signal-catalog binding; applies to both on-board and off-board fleets — see spec `2026-06-10-cms-fleet-source-classifier-redesign`) |
...
- **Frontend UX**: 
  ...
  - Fleet creation form three-layer source classifier (Layer 1: On-board (FWE) / Off-board (cloud); Layer 2: OEM provider — only for off-board; Layer 3: decoder manifest [always] + transform manifest [off-board only]). Spec: `2026-06-10-cms-fleet-source-classifier-redesign`. Forward-compat: Layer 2 is a dropdown that scales additively; adding OEMn does not require top-level UI churn.
...
- **Fleet-creation Lambda persists client `data_source` + manifest IDs**: As of spec `2026-06-10-cms-fleet-source-classifier-redesign` (closed 2026-06-10), the main-API `POST /fleets` Lambda at `main_api/index.py` extracts and validates `data_source` (closed enum w/ dual-read), `decoder_manifest_id`, and `transform_manifest_id` from the request body, persists all three on the fleet item, and returns `400` on invalid input. The previous "silently drops `data_source`" gap (filed as P3 carry-forward from `2026-06-09-cms-data-source-model-refactor`) is closed by this spec via subsumption.
...
|---|---|---|
| `TriggeredEvent` | `autonomic.ext.event.TriggeredEvent` | Emits decoded TriggeredEvent JSON for both `well_known_label` and `string_label` event variants. The `stringLabel` field is preserved on the decoded JSON for downstream manifest matching (see `stringLabelEndsWith` predicate). |
...
4. Emits the result as camelCase JSON via `MessageToJson(preserving_proto_field_name=False)`, or as `_raw_hex` fallback on unpack failure

### Path-ε string_label TriggeredEvent decode (spec `2026-06-09-cms-oem1-dtc-engine-light-pipeline`)

OEM1 emits VHA Custom Diagnostic Events as TriggeredEvents whose `stringLabel` ends with `:custom:vha-diagnostics-processed-event`. These records previously fell back to `_raw_hex` (parent spec assumption that string_label TriggeredEvents are external-tenant noise). The DTC engine-light spec lifts that discard: all TriggeredEvent variants — `well_known_label` AND `string_label` — now decode to JSON. The decoded `stringLabel` is preserved verbatim so the manifest engine's `stringLabelEndsWith` predicate can match on the suffix and route to the `cms.vha_diagnostic_event` event_type. See `services/data_processing/manifests/oem1-transform.json` event_mappings entry for the canonical-event extraction schema, and `MaintenanceProcessor.handleCanonicalIndicatorEvent` for the 4-state (ACTIVE / ACTIVE_NO_DTC / CLEARED / DTC_CLEARED_INDICATOR_ACTIVE) dtc-history materialization with `source: oem1-uds-dtc`.
...
## Way B Kafka message envelope (top-level fields)

Under `OEM1_EMIT_TARGET=kafka` (Way B — the mode deployed in staging/prod), each
feed event is emitted as one raw protobuf-as-JSON Kafka message built by
`_kafka_raw_payload`. The top-level envelope is:

```json
{
  "typedData": { "@type": "<type-url>", "value": { ... } },
  "shard_key": "aui:asset:vehicle/<uuid>",
  "vin": "1FTBR1C88RKA27079",
  "timestamp": "2026-06-11T17:00:00.000Z",
  "oem_source": "oem1",
  "reference_hex": "0000000000000000"
}
```

| Field | Notes |
|---|---|
| `typedData` | `@type` (proto type-URL) + decoded camelCase `value`. |
| `shard_key` | Verbatim feed `shard_key` (`aui:asset:<vehicle\|device>/<uuid>`). Resolver fallback input. |
| `vin` | **Self-describing join key (added 2026-06-11).** The real VIN extracted from the feed's `asset_info`, or `null` when the feed carries no usable VIN. |
| `timestamp` | ISO-8601 from the feed-event timestamp. |
| `oem_source` | Always `"oem1"` (manifest selector). |
| `reference_hex` | First 16 chars of the feed-event reference hex. |

**VIN in Kafka payload is the canonical join key.** `OEMTelemetryProcessor`
prefers the top-level `vin` field over `shard_key` resolution: if `vin` is
present (non-null, non-empty, not `aui:`-prefixed) it is used directly, so trips
and downstream rows are VIN-keyed at the source. Only when the `vin` field is
absent/null does the processor fall back to resolving the `shard_key` device
UUID against the vehicles table via `deviceToVehicleResolver`
(`aui_asset_resolve`). The fallback is retained defensively for legacy in-flight
messages and future feed-format changes.

The Way B run loop also calls `auto_register.handle_unknown_vin` (throttled to
≤1 DynamoDB write per VIN per hour, best-effort) to populate the vehicles
table's `oem1_device_uuid` device→VIN mapping for operator-visibility UI
affordances. This is decoupled from pipeline correctness — the VIN already
travels in the message — so a throttled or failed auto-register write never
affects VIN-keyed trip production. See
`issues/2026-06-11-oem1-kafka-path-skips-auto-register/` and
`docs/architecture/oem1-fleet-lifecycle.md` § Device-VIN binding.
