...
def _make_feed_event_proto(payload_msg, shard_key: str = "aui:asset:vehicle/test-uuid-001", vin: str | None = None):
    """Build a real consumer_pb2.FeedEvent wrapping payload_msg.

    If `vin` is provided, populate asset_info.vehicle_asset_info.vin so the
    Way B payload's top-level `vin` field is exercised.
    """
...
class TestKafkaEmitShape:
    ...
    def _raw(self, payload_msg, shard_key="aui:asset:vehicle/test-uuid", vin=None):
        """Build a FeedEvent and call _kafka_raw_payload()."""
        from connector import OEM1Connector
        evt = _make_feed_event_proto(payload_msg, shard_key=shard_key, vin=vin)
        return OEM1Connector._kafka_raw_payload(evt)

    def test_top_level_fields_present(self):
        """Way B message has typedData, shard_key, vin, timestamp, oem_source, reference_hex."""
        out = self._raw(_make_speed_metric())
        assert out is not None
        assert set(out.keys()) == {"typedData", "shard_key", "vin", "timestamp", "oem_source", "reference_hex"}

    def test_vin_present_when_asset_info_carries_it(self):
        """Real VIN from asset_info.vehicle_asset_info.vin lands in the top-level `vin` field."""
        out = self._raw(_make_speed_metric(), vin="1FTBR1C88RKA27079")
        assert out["vin"] == "1FTBR1C88RKA27079"

    def test_vin_none_when_no_asset_info(self):
        """No asset_info → vin is None (so Flink engages its shard_key resolver fallback)."""
        out = self._raw(_make_speed_metric())
