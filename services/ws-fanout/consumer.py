...
# The API Gateway Management API is HTTPS. The WebSocketEndpoint CFN output (and
# thus WS_API_ENDPOINT) is wss://{id}.execute-api.{region}.amazonaws.com/live —
# normalize to https:// or boto3 raises "Not supported URL scheme wss".
_MGMT_ENDPOINT = WS_API_ENDPOINT.replace('wss://', 'https://', 1)
apigw = boto3.client('apigatewaymanagementapi', endpoint_url=_MGMT_ENDPOINT)
...
def run():
    ...
    for message in consumer:
        try:
            ...
            # Get connected clients for this fleet, plus all-fleet (admin)
            # connections stored under the '*' partition (Option 2 — spec
            # 2026-06-16-cms-ui-realtime-websocket-wiring). Admin dashboards
            # subscribe to every fleet via a single '*' connection.
            connections = get_connections(fleet_id) + get_connections('*')
            if not connections:
                continue
