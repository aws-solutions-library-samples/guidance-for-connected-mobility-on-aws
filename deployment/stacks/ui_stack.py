...
class UIStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, 
        ...
        self.map = location.CfnMap(
            self, "CMSVehicleMap",
            map_name=f"{construct_id}-vehicle-map",
        ...
        self.place_index = location.CfnPlaceIndex(
            self, "CMSPlaceIndex",
            index_name=f"{construct_id}-place-index",
            data_source="Esri",
        ...
        #   1. `-c frontendBucketName=...` — pin to an exact physical name
        #      (used to freeze the current bucket on cms-prod-ui and stop
        #      CDK from replacing it on every synth).
        #   2. Fallback: `{construct_id}-frontend-{account}-{timestamp}`.
        #      The timestamp drifts on every synth, so this path should only
        #      be used on first-ever deploys to a region. Pass the resulting
        #      bucket name back as `frontendBucketName` on subsequent deploys.
        frontend_bucket_name = (
            self.node.try_get_context("frontendBucketName")
            or f"{construct_id}-frontend-{self.account}-{int(time.time())}"
        )
                ...
                "AppAccess": iam.PolicyDocument(
                    statements=[
                        ...
                        iam.PolicyStatement(
                            ...
                            resources=[
                                "arn:aws:s3:::cms-prod-vfo-knowledge-base",
                                "arn:aws:s3:::cms-prod-vfo-knowledge-base/*",
                                f"arn:aws:s3:::cms-{construct_id.split('-')[1]}-vfo-knowledge-base",
                                f"arn:aws:s3:::cms-{construct_id.split('-')[1]}-vfo-knowledge-base/*",
                            ]
                        )
                    ]
