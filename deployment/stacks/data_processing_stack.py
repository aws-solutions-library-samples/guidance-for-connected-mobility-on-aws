...
class DataProcessingStack(Stack):
    def __init__(self, scope: Construct, construct_id: str, **kwargs):
        ...
        #
        # The CMS User Pool is always imported from the ui_stack via a
        # cross-stack reference. `CMS_EXTRA_USER_POOL_IDS` (comma-separated)
        # lets iOS-only or partner pools also access the API.
        cms_extra_pool_ids = [
        ...
        self.api_lambda = lambda_.Function(
            ...
            environment={
                ...
                'FLEETS_TABLE_NAME': f'cms-{deployment_stage}-storage-fleets',
            },
        ...
        # Read-only access to fleets table for live vehicleCount computation
        self.api_lambda.add_to_role_policy(iam.PolicyStatement(
            actions=['dynamodb:GetItem'],
            resources=[f'arn:aws:dynamodb:{self.region}:{self.account}:table/cms-{deployment_stage}-storage-fleets'],
        ))
        
        # ===================================================================
        # 6. API Gateway REST API
        # ===================================================================
        # Import the CMS User Pool from the ui_stack via cross-stack reference.
        # Partner pools in CMS_EXTRA_USER_POOL_IDS are added alongside.
        ui_construct_id = f'cms-{deployment_stage}-ui'
        cms_user_pool = cognito.UserPool.from_user_pool_arn(
            self, 'ImportedCMSUserPool',
            user_pool_arn=Fn.import_value(f'{ui_construct_id}-user-pool-arn'),
        )
        authorized_pools = [cms_user_pool] + [
