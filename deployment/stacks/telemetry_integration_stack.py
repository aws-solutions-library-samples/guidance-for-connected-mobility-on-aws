...
def lambda_handler(event, context):
    try:
        ...
        vpc_policy = iam.ManagedPolicy(
            ...
            statements=[
                ...
                iam.PolicyStatement(
                    ...
                    resources=[
                        f"arn:aws:s3:::cms-{deployment_stage}-telemetry-backup-{self.region}-{self.account}",
                        f"arn:aws:s3:::cms-{deployment_stage}-telemetry-backup-{self.region}-{self.account}/*"
                    ]
                ),
        ...
        bucket_name = f"cms-{deployment_stage}-telemetry-backup-{self.region}-{self.account}"
