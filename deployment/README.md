...
- `cms-{stage}-flink` - Stream processing applications including:
  ...
  - **TripProcessor** — detects trips from ignition signal (FWE) or lifecycle events (MQTT Direct). Cross-OEM canonical-event passthrough as of v0.2.0. Domain topics vehicleId-keyed; runs at parallelism=3 (scaled v0.3.1+).
  ...
  - **Observability**: 30 CloudWatch alarms (6 processors × 5 metrics) wired to a KMS-encrypted SNS topic `cms-{stage}-flink-alarms`:
    - Lag: `records_lag_max > 10,000 for 3×5min`
    - Downtime: `downtime > 0 for 1×1min`
    - Restarts: `fullRestarts > 2 in 15min`
    - Failed checkpoints: `numberOfFailedCheckpoints > 0 for 1×5min`
    - CPU: `containerCPUUtilization > 75% for 3×5min`
  - Log level: INFO (reduced from DEBUG to lower per-record CPU cost on hot path)
