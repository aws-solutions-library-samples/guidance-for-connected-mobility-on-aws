...
# Detects whether an S3 bucket name carries the canonical region+account
# suffix introduced by the 2026-06-03/04 cross-region naming work. Two
# orderings observed in production:
#   - cms-{stage}-storage-invoices-{region}-{account}     (storage stack)
#   - cms-{stage}-vfo-knowledge-base-{region}-{account}   (vfo-kb)
#   - cms-{stage}-ui-frontend-{account}-{region}          (ui FrontendBucket)
# Suffixed names CANNOT collide cross-region with another region's deploy
# (each region stamps a unique suffix). Unsuffixed names ARE real
# cross-region collision risks and must block the harness.
#
# Region pattern: 2-3 letter geo (us, eu, ap, sa, af, me, ca, il, ...)
# + 1+ word groups + digit. Account: 12 digits (AWS canonical).
# Pattern is anchored to end-of-string and matches either ordering.
_REGION_ACCOUNT_SUFFIX = re.compile(
    r"-(?:"
    r"[a-z]{2,3}(?:-[a-z]+)+-\d+-\d{12}"   # ...-{region}-{account}
    r"|"
    r"\d{12}-[a-z]{2,3}(?:-[a-z]+)+-\d+"   # ...-{account}-{region}
    r")$"
)


# Detects CDK L2 auto-named bucket patterns. CDK generates bucket names of
# the shape `{stack-name}-{lc-construct-id}{8-hex-hash}-{12-char-random}`
# where the 12-char random tail is unique-per-deploy. Two buckets from
# the SAME construct in different regions get DIFFERENT random tails
# (the random tail is a function of stack-id + region + account + logical
# id), so cross-region name collision is structurally impossible for
# CDK auto-named buckets — even though the names lack the canonical
# `-{region}-{account}` suffix. Without this carve-out, the preflight
# false-positives on every L2 auto-named bucket alive in another region
# of the same account, e.g.
#   `cms-staging-flink-flinkjarbucketd8dc3634-5f4bgm0yqt3q` (us-west-2)
#   `cms-staging-storage-datalakebucketcc682ad4-niub1nvtbtki` (us-west-2)
# Issue: surfaced by Tokyo clean redeploy run 2026-06-17 (spec
# `2026-06-17-cms-tokyo-clean-redeploy`). The us-west-2 bucket above is
# a live FlinkJarBucket of the user's parallel UAT environment; deleting
# it would break UAT, and the Tokyo deploy CANNOT collide with it
# because Tokyo's CDK will generate a different 12-char tail.
_CDK_AUTO_NAME_TAIL = re.compile(
    r"-[a-z][a-z0-9]+[a-f0-9]{8}-[a-z0-9]{12,13}$"
)


def _has_region_account_suffix(bucket_name: str) -> bool:
    """Return True if the bucket name ends with -{region}-{account} or
    -{account}-{region}, indicating CDK-stamped cross-region-safe naming.
    Returns False for unsuffixed names (regression class — real collision
    risk on cross-region deploy)."""
    return bool(_REGION_ACCOUNT_SUFFIX.search(bucket_name))


def _has_cdk_auto_name(bucket_name: str) -> bool:
    """Return True if the bucket name matches CDK L2 auto-naming
    pattern `...{8hex}-{12chars}`. Such buckets are unique per
    region/account/stack via the per-deploy random tail and CANNOT
    collide cross-region. Treated as not-collision-risk by the
    preflight."""
    return bool(_CDK_AUTO_NAME_TAIL.search(bucket_name))
...
def check_s3_buckets(region: str, stage: str, auto_clean: bool) -> int:
    ...
    global_namespace_collisions: list[tuple[str, str]] = []  # unsuffixed cross-region — REAL collision risk; blocks
    global_namespace_warnings: list[tuple[str, str]] = []    # suffixed cross-region — informational only
    ...
    for b in s3.list_buckets()["Buckets"]:
        ...
        elif _has_region_account_suffix(name) or _has_cdk_auto_name(name):
            # Suffixed cross-region buckets cannot collide (each region's
            # deploy stamps a unique suffix). CDK L2 auto-named buckets
            # (matching `-{8hex}-{12chars}$`) likewise cannot collide
            # because the random tail is per-deploy unique. Surface
            # both as informational only.
            global_namespace_warnings.append((name, loc))
        else:
            # Unsuffixed cross-region bucket would collide on deploy here.
            # This is a real regression and must block the harness.
            global_namespace_collisions.append((name, loc))
    ...
    # Phase 2a — global-namespace COLLISIONS (unsuffixed cross-region
    # buckets). These ARE real collision risks: any cms-{stage}-* bucket
    # that does NOT carry the canonical `-{region}-{account}` (or
    # `-{account}-{region}`) suffix will collide with this region's
    # synthesis. Block here so the harness fails in ~9s at preflight
    # rather than ~34min into deploy_all CFN early-validation.
    # See backlog row "Preflight S3-collision blocker" (closed 2026-06-07).
    if global_namespace_collisions:
        warn(f"S3 cross-region COLLISION (unsuffixed cms-{stage}-* buckets): "
             f"{len(global_namespace_collisions)}",
             "would collide on deploy in this region")
        for b, b_region in global_namespace_collisions:
            print(f"      - {b} (alive in: {b_region}) — UNSUFFIXED")
        print(f"      ⚠️  These bucket names lack the canonical "
              f"-{{region}}-{{account}} suffix introduced by 2026-06-03/04 "
              f"naming work; deploying cms-{stage}-* in {region} WILL "
              f"hit `BucketAlreadyExists` ~34min into deploy_all.")
        print(f"      → To resolve: tear down the other region's deploy first, "
              f"OR amend the offending bucket's CDK definition to include "
              ...
              f"Auto-clean is intentionally NOT offered for cross-region "
              f"buckets — they may belong to a live deployment.")

    # Phase 2b — global-namespace WARNINGS (suffixed cross-region
    # buckets). These do NOT collide because each region's deploy stamps
    # a unique suffix. Surface as informational only so the operator
    # sees the cross-region footprint without blocking.
    if global_namespace_warnings:
        print(f"  ℹ️  S3 buckets matching cms-{stage}-* in OTHER regions "
              f"(suffix-stamped, NOT collision-risk): "
    ...
    return len(regional_blockers) + len(global_namespace_collisions)
