...
def check_s3_buckets(region: str, stage: str, auto_clean: bool) -> int:
    ...
    if not blockers:
        ...
        return 0
    warn(f"S3 buckets in {region}: {len(blockers)}", "names will collide on deploy")
    for b in blockers:
        print(f"      - {b}")
    if auto_clean:
        for b in blockers:
...
def check_bedrock_agents(region: str, stage: str, auto_clean: bool) -> int:
    try:
        ...
        ok("Bedrock agents (cms-*): 0 (or service unavailable)")
        return 0
...
def main() -> int:
    ...
    ap.add_argument("--auto-clean", action="store_true",
    ...
    print(f"      Re-run with --auto-clean to attempt deletion, or fix manually.")
    return 1
