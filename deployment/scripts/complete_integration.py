...
def update_flink_app(app_name: str, bootstrap_servers: str, secret_arn: str):
    ...
    processor_types = {
        ...
        "simulator-preprocessor": "SimulatorPreprocessor",
        "fw-telemetry-processor": "FWTelemetryProcessor",
        "oem-telemetry-processor": "OEMTelemetryProcessor",
        "campaign-sync-processor": "CampaignSyncProcessor",
        "geofence-processor": "GeofenceProcessor",
    }
...
def create_kafka_topics_via_api(deployment_stage: str):
    ...
    try:
        ...
        # These MUST exist before consumers start. IoT→MSK rules use SCRAM auth, which
        # lacks the CreateTopics ACL on MSK — so auto.create.topics.enable does not
        # help the IoT producer path. Flink consumers crash in a restart loop if their
        # source topics don't exist yet. Creating topics up-front avoids both problems.
        # Topic names MUST match what the Flink Java code uses (see modules/flink/src/
        # main/java/com/cms/telemetry/*.java setTopic / setTopics calls).
        topics = [
            # Ingestion topics (produced by IoT rules via SCRAM; consumed by Flink IAM)
            {"name": "cms-telemetry-raw",          "partitions": 3, "replication": 2},  # CMS sim IoT rule
            {"name": "fw-telemetry-raw",           "partitions": 3, "replication": 2},  # FWE IoT rule
            {"name": "cms-telemetry-oem",          "partitions": 3, "replication": 2},  # OEM ingestion
            # Preprocessed (produced by SimulatorPreprocessor/FWTelemetryProcessor/OEMTelemetryProcessor)
            {"name": "cms-telemetry-preprocessed", "partitions": 3, "replication": 2},
            # Domain-routed (produced by EventDrivenTelemetryProcessor, consumed by TripProcessor/SafetyProcessor/MaintenanceProcessor/TelemetryDataProcessor)
            {"name": "cms-telemetry-processed",    "partitions": 3, "replication": 2},
            {"name": "cms-telemetry-trips",        "partitions": 3, "replication": 2},
            {"name": "cms-telemetry-safety",       "partitions": 3, "replication": 2},
...
def add_vpc_configuration_to_flink_applications(deployment_stage: str):
    ...
    applications = [
        ...
        f"cms-{deployment_stage}-flink-simulator-preprocessor",
        f"cms-{deployment_stage}-flink-fw-telemetry-processor",
        f"cms-{deployment_stage}-flink-oem-telemetry-processor",
        f"cms-{deployment_stage}-flink-campaign-sync-processor",
        f"cms-{deployment_stage}-flink-geofence-processor",
    ]
...
def main():
    ...
    # WARNING: update_flink_app() does a FULL replace of the PropertyMap. Any app-specific
    # env vars set by CDK (e.g. TABLE_NAME, REDIS_ENDPOINT, TELEMETRY_TABLE_NAME, event/signal
    # catalog tables) will be wiped. Only include apps here if you've verified update_flink_app
    # writes back all the env vars they need — otherwise prefer to leave CDK's synth-time
    # config in place. To restore full config, redeploy the flink stack.
