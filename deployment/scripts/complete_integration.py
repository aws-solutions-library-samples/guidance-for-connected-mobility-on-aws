...
def create_kafka_topics_via_api(deployment_stage: str):
    """Create the canonical CMS MSK topic set, idempotently.

    Delegates to ``deployment/scripts/create_msk_topics.py`` for a
    single source of truth on the canonical list and the SASL/IAM
    creation logic.

    Replaces a previous implementation that called
    ``boto3.client('kafka').create_topic(...)`` — that API does NOT
    exist on the AWS MSK control plane. Topic management requires the
    Kafka admin protocol against the brokers (security_protocol=
    SASL_SSL, sasl_mechanism=OAUTHBEARER with the AWS-MSK-IAM signer).

    The delegating subprocess MUST run from a host that has network
    reach to the MSK broker SASL/IAM port (9098). Inside the MSK VPC,
    or via an SSM port-forward (in which case the caller exports
    ``BOOTSTRAP_SERVERS=localhost:<local-port>`` before invoking this
    function).
    """
    import subprocess
    import sys as _sys
    ...
    script = os.path.join(here, "create_msk_topics.py")

    env = {**os.environ}
    env["DEPLOYMENT_STAGE"] = deployment_stage
    env["AWS_REGION"] = REGION

    print(f"🔄 Creating canonical MSK topics via {script}")
    try:
        subprocess.check_call([_sys.executable, script], env=env)
        return True
    except subprocess.CalledProcessError as exc:
        print(f"❌ Topic creation failed (rc={exc.returncode})")
        return False
    except FileNotFoundError:
        print(f"❌ Topic creation script not found: {script}")
        return False
