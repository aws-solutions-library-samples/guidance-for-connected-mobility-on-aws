...
# ---------------------------------------------------------------------------
# Step 2 — list RUNNING tasks for the family
# ---------------------------------------------------------------------------
...
# Parse taskArns. Empty list → exit 0 cleanly.
TASK_ARNS=$(printf '%s' "$LIST_JSON" \
    | python3 -c 'import json,sys; arns = json.load(sys.stdin).get("taskArns", []); print("\n".join(arns))')
