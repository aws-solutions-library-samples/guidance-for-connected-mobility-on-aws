...
# 5. Docker daemon (CDK uses Docker for some Lambda asset bundling)
log "Check 5: Docker daemon is running"
if docker info >/dev/null 2>&1; then
  ok "  Docker info OK"
else
  err "  Docker not running. Start Docker Desktop or 'brew services start docker'."
  failures=$((failures+1))
fi
