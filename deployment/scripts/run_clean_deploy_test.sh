...
#  TRAP. `teardown_region_force.py` + `audit_region_orphans.py` —
#       runs unconditionally on EXIT, even if any earlier phase
#       failed (set -e short-circuit). Trap also emits report.json.
#
...
teardown_and_audit() {
  ...
  log "═══════════════════════════════════════════════════════════════"
  log "Trap fired (exit=${exit_code}); running unconditional teardown + audit"
  log "═══════════════════════════════════════════════════════════════"

  # Teardown — always runs. Continue even on failure so audit still runs.
