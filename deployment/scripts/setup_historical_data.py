...
def main():
    ...
    if batch_mode:
        ...
        if os.environ.get('HISTORY_YEARS'):
            days = int(os.environ['HISTORY_YEARS']) * 365
            print(f"Using HISTORY_YEARS={os.environ['HISTORY_YEARS']} → {days} days")
        elif os.environ.get('HISTORY_DAYS'):
            days = int(os.environ['HISTORY_DAYS'])
        else:
            days = 30
        num_fleets = 5
