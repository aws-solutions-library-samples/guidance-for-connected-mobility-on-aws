...
def seed_extra_templates():
    ...
    try:
        ...
        if not default_signals_to_collect:
            raise RuntimeError(
                "Signal catalog scan returned 0 active signals. "
                "Run `make seed-signal-catalog` first, then retry."
            )
        print(f"  default signalsToCollect: {len(default_signals_to_collect)} active signals from catalog (excluding UDS DTC IDs ≥ 900)")
    except RuntimeError:
        raise
    except Exception as e:
        raise RuntimeError(f"Could not load signal catalog: {e}. Run seed-signal-catalog first.") from e
