...
def delete_orphaned_log_groups(region, stage):
    """Delete log groups that persist after stack deletion (RETAIN policy)."""
    ...
    try:
        ...
        print(f"  ⚠️  Could not list log groups: {e}")


def delete_orphaned_tables(region, stage):
    """Delete DynamoDB tables that persist after stack deletion (RETAIN policy)."""
...
def main():
    ...
    # 2. Empty S3 buckets and clean orphaned resources
    print("\n📌 Step 2: Emptying S3 buckets...")
    empty_s3_buckets(cf, region, all_stacks)
    print("\n📌 Step 2b: Deleting orphaned log groups...")
    delete_orphaned_log_groups(region, stage)
    print("\n📌 Step 2c: Deleting orphaned DynamoDB tables...")
    delete_orphaned_tables(region, stage)
