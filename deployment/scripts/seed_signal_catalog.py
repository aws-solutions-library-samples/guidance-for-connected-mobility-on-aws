...
# 25 signals required by the per-event signal contract table (MUST-ADD from Phase 1).
...
EVENT_CONTRACT_SIGNALS = [
    # (signal_name, signal_group, description, json_field, data_type, unit, vss_path)
    ("AirbagSystemStatus",         "safety",      "Airbag system status/warning indicator (0=OK, 1=warning)",                         "airbag_warn",                 "boolean", None,  "Vehicle.Safety.Airbag.SystemStatus"),
    ("ABSFaultStatus",             "safety",      "ABS fault/activation status (0=inactive, 1=active/fault)",                         "abs_act",                     "boolean", None,  "Vehicle.ADAS.ABS.FaultStatus"),
    ("LightingSystemStatus",       "body",        "Lighting system fault flag (0=OK, 1=fault)",                                        "lighting_system_fault",       "boolean", None,  "Vehicle.Body.Lights.SystemFault"),
    ("SteeringSystemStatus",       "chassis",     "Steering system fault flag (0=OK, 1=fault)",                                        "steering_fault",              "boolean", None,  "Vehicle.Chassis.Steering.FaultStatus"),
    ("MILStatus",                  "diagnostics", "Malfunction indicator lamp / DTC active status (0=off, 1=on)",                      "dtc_codes_active",            "boolean", None,  "Vehicle.Diagnostics.MIL.Status"),
    ("MisfireCount",               "powertrain",  "Engine misfire event count",                                                        "misfire_count",               "integer", None,  "Vehicle.Powertrain.Engine.MisfireCount"),
    ("FuelMixtureBank1",           "powertrain",  "Fuel mixture ratio bank 1 (lambda/short-term fuel trim)",                           "fuel_mixture_bank1",          "float",   None,  "Vehicle.Powertrain.Engine.FuelMixtureBank1"),
    ("PCMStatus",                  "diagnostics", "Powertrain control module fault active (0=OK, 1=fault)",                            "pcm_fault_active",            "boolean", None,  "Vehicle.Diagnostics.PCM.FaultStatus"),
    ("TransmissionStatus",         "powertrain",  "Transmission fault active flag (0=OK, 1=fault)",                                    "transmission_fault_active",   "boolean", None,  "Vehicle.Powertrain.Transmission.FaultStatus"),
    ("BrakeSystemStatus",          "chassis",     "Brake system fault flag (0=OK, 1=fault)",                                           "brake_system_fault",          "boolean", None,  "Vehicle.Chassis.Brake.SystemFault"),
    ("TractionControlStatus",      "safety",      "Traction control fault/active status (0=OK/inactive, 1=active/fault)",              "traction_control",            "boolean", None,  "Vehicle.ADAS.TractionControl.FaultStatus"),
    ("EvapLeakDetected",           "emissions",   "EVAP system leak detected (0=no leak, 1=leak detected)",                            "evap_leak_detected",          "boolean", None,  "Vehicle.Powertrain.Emissions.EVAP.LeakDetected"),
    ("CatalystEfficiency",         "emissions",   "Catalyst efficiency percentage (below threshold = failing)",                         "catalyst_efficiency",         "float",   "%",   "Vehicle.Powertrain.Emissions.CatalystEfficiency"),
    ("PCMCommStatus",              "diagnostics", "PCM communication fault/lost-comm status (0=OK, 1=lost)",                           "pcm_comm_status",             "boolean", None,  "Vehicle.Diagnostics.PCM.CommStatus"),
    ("ECMDataValid",               "diagnostics", "ECM data valid flag (0=invalid/fault, 1=valid)",                                     "ecm_data_valid",              "boolean", None,  "Vehicle.Diagnostics.ECM.DataValid"),
    ("ECUInternalStatus",          "diagnostics", "ECU internal fault flag (0=OK, 1=internal fault)",                                   "ecu_internal_flag",           "boolean", None,  "Vehicle.Diagnostics.ECU.InternalFault"),
    ("PowertrainMalfunctionStatus","powertrain",  "Powertrain system malfunction flag (0=OK, 1=malfunction)",                           "powertrain_malfunction",      "boolean", None,  "Vehicle.Powertrain.MalfunctionStatus"),
    ("ChargeSystemStatus",         "ev_charging", "Charging system fault flag (0=OK, 1=fault)",                                        "charge_system_fault",         "boolean", None,  "Vehicle.Powertrain.TractionBattery.Charging.FaultStatus"),
    ("WheelSpeedSensorLF",         "chassis",     "Left-front wheel speed sensor fault (0=OK, 1=fault)",                               "wheel_speed_sensor_lf_fault", "boolean", None,  "Vehicle.Chassis.Axle.Row1.Wheel.Left.WheelSpeedSensor.Fault"),
    ("WheelSpeedSensorRF",         "chassis",     "Right-front wheel speed sensor fault (0=OK, 1=fault)",                              "wheel_speed_sensor_rf_fault", "boolean", None,  "Vehicle.Chassis.Axle.Row1.Wheel.Right.WheelSpeedSensor.Fault"),
    ("WaterInFuelStatus",          "maintenance", "Water-in-fuel sensor status (0=clear, 1=water detected)",                           "water_in_fuel",               "boolean", None,  "Vehicle.Powertrain.FuelSystem.WaterInFuel"),
    ("CamshaftSensorStatus",       "powertrain",  "Camshaft position sensor fault (0=OK, 1=fault)",                                    "camshaft_sensor_fault",       "boolean", None,  "Vehicle.Powertrain.Engine.CamshaftSensor.Fault"),
    ("TrailerBrakeStatus",         "chassis",     "Trailer brake system disconnected/fault (0=OK, 1=fault)",                           "trailer_brake_fault",         "boolean", None,  "Vehicle.Chassis.Trailer.Brake.FaultStatus"),
    ("BrakeFluidLevel",            "chassis",     "Brake fluid level percentage (below threshold = low)",                              "brake_fluid_level",           "float",   "%",   "Vehicle.Chassis.Brake.FluidLevel"),
]


def seed_contract_signals():
    """Idempotently seed the 25 event-contract gap signals.

    Uses attribute_not_exists condition so re-runs are safe.
    In dry-run mode prints 'WOULD SEED: <signal_name>' for each.
    """
    if DRY_RUN:
        for signal_name, group, description, json_field, data_type, unit, vss_path in EVENT_CONTRACT_SIGNALS:
            print(f"WOULD SEED: {signal_name} (json_field={json_field}, group={group})")
        return
    ...
    for signal_name, group, description, json_field, data_type, unit, vss_path in EVENT_CONTRACT_SIGNALS:
        ...
        try:
            ...
            seeded += 1
            print(f"  ✅ Seeded: {signal_name} (json_field={json_field})")
        except dynamodb.meta.client.exceptions.ConditionalCheckFailedException:
            skipped += 1
            print(f"  ⏭  Already exists: {signal_name}")
