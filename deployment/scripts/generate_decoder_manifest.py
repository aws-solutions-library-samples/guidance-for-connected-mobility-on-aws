...
Commit workflow (stack-managed deploy)
---------------------------------------
The CDK flink stack (``deployment/stacks/flink_stack.py``) uploads
``deployment/fwe-config/DecoderManifest.bin`` into the flink jar bucket
under ``fwe-config/`` on every ``cdk deploy cms-<stage>-flink``. This
means the regenerated binary **MUST be committed** to
``deployment/fwe-config/DecoderManifest.bin`` so CDK has a local artifact
to include in the BucketDeployment asset. The out-of-band S3 upload path
in this script is kept for emergency/production runbook use only.

Regen + commit workflow::

    DRY_RUN=1 python3 deployment/scripts/generate_decoder_manifest.py
    cp DecoderManifest.bin deployment/fwe-config/DecoderManifest.bin
    git add deployment/fwe-config/DecoderManifest.bin
    git commit -m "chore(fwe): regenerate DecoderManifest.bin"
...
def dump_signal_names_from_seed(seed_path: str, out_path: str) -> int:
    """Write a text file listing all signal_names from the signal catalog seed JSON.

    This runs fully locally — no AWS calls required.  Use it to verify that
    every MUST-ADD signal from the contract table is present in the seed before
    applying changes to live AWS:

        python3 deployment/scripts/generate_decoder_manifest.py --dump-signal-names \\
            --out /tmp/signal_names.txt
        grep AirbagSystemStatus /tmp/signal_names.txt

    Each line is: ``<signal_name>  json_field=<json_field>  source=<source>``
    """
    ...
    lines = []
    for r in sorted(rows, key=lambda x: x.get("signal_name", "")):
        name = r.get("signal_name", "")
        jf = r.get("json_field", "")
        src = r.get("source", "")
        lines.append(f"{name}  json_field={jf}  source={src}")

    content = "\n".join(lines) + "\n"
    with open(out_path, "w") as f:
        f.write(content)

    print(f"Wrote {len(rows)} signal names to {out_path}")
    return 0
...
def main(argv=None) -> int:
    ...
    p.add_argument("--dump-signal-names", action="store_true",
                   help="Write a text list of all signal names from the seed JSON "
                        "to --out (no AWS calls, no protobuf). Use with grep to "
                        "verify MUST-ADD signals are present before live apply.")
    args = p.parse_args(argv)

    if args.dump_signal_names:
        repo_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
        seed_path = os.path.join(repo_root, "deployment", "scripts", "signal_catalog_seed.json")
        return dump_signal_names_from_seed(seed_path, args.out)
