...
# Sibling stacks that contribute endpoints to runtimeConfig.json. Each tuple is
# (stack-name-template, runtimeConfig-field-name, optional-CFN-output-key).
# When the stack exists, the runtimeConfig field MUST be non-empty.
# When the stack does NOT exist, the runtimeConfig field MAY be empty (legitimately absent).
# This is the contract the runtimeConfig endpoint-race hardening asserts (issue
# 2026-06-15-runtimeconfig-endpoint-race-hardening).
_RUNTIME_CONFIG_SIBLING_ENDPOINTS = [
    ("cms-{stage}-data-processing", "dataProcessingApiEndpoint", "APIEndpoint"),
    ("cms-{stage}-simulation",      "simulationApiEndpoint",     "SimulationApiUrl"),
    ("cms-{stage}-commands",        "commandsApiEndpoint",       "CommandsApiUrl"),
]


def _stack_exists(region: str, stack_name: str) -> bool:
    """Return True iff CloudFormation describe-stacks returns a stack."""
    cfn = boto3.client("cloudformation", region_name=region)
    try:
...
@pytest.mark.e2e
def test_S4b_runtime_config_endpoint_integrity(region: str, stage: str, cf_url: str) -> None:
    """S4b — runtimeConfig endpoint integrity: every sibling stack that IS
    deployed has its runtimeConfig field populated with a non-empty https URL.

    This guards the endpoint-race class of bug surfaced in Tokyo on 2026-06-15
    (issue ``2026-06-15-tokyo-runtime-config-data-processing-endpoint-empty``):
    when the UI stack synthed before its sibling stacks existed, every
    sibling-endpoint resolver fell back to ``""`` silently and shipped to S3.

    The contract:

    1. For each ``(stack_template, field, output_key)`` in
       ``_RUNTIME_CONFIG_SIBLING_ENDPOINTS``: if the stack exists in the
       target region, the runtimeConfig field MUST be a non-empty https URL.
       If the stack does not exist, the field MAY be empty.
    2. If runtimeConfig.json carries the new ``_unresolved_endpoints``
       provenance list, every key in it MUST correspond to a sibling stack
       that does NOT currently exist (i.e. unresolved == legitimately absent).
       This catches the case where the UI was synthed pre-sibling-deploy AND
       no post-deploy ``regenerate-runtime-config`` ran to refresh the bake.
    3. If runtimeConfig.json carries ``_resolved_at``, it must parse as
       ISO-8601 (best-effort sanity check, not a freshness gate).
    """
    url = f"{cf_url}/runtimeConfig.json"
    resp = requests.get(url, timeout=15)
    assert resp.status_code == 200, (
    ...
    failures: List[str] = []
    for stack_template, field, output_key in _RUNTIME_CONFIG_SIBLING_ENDPOINTS:
        stack_name = stack_template.format(stage=stage)
        exists = _stack_exists(region, stack_name)
        value = body.get(field, "")
        if exists:
            if not isinstance(value, str) or not value:
                failures.append(
                    f"{field!r}: stack {stack_name!r} EXISTS in {region} but "
                    f"runtimeConfig.{field}={value!r} (expected non-empty https URL "
                    f"sourced from CFN output {output_key!r}). "
                    f"Run `make regenerate-runtime-config DEPLOYMENT_STAGE={stage} "
                    f"AWS_REGION={region}` to fix."
                )
            elif not value.startswith("https://"):
                failures.append(
                    f"{field!r}: stack {stack_name!r} exists but "
                    f"runtimeConfig.{field}={value!r} is not an https URL"
                )
        else:
            # Stack absent — empty field is legitimate. No assertion.
            pass

    # Provenance check: any key in _unresolved_endpoints must correspond to
    # an absent stack. A key in this list whose stack DOES exist is a real
    # bug — the synth-time empty fallback was never repaired by a post-deploy
    # regenerate-runtime-config run.
    unresolved = body.get("_unresolved_endpoints")
    if isinstance(unresolved, list):
        field_to_stack = {
            field: stack_template.format(stage=stage)
            for stack_template, field, _ in _RUNTIME_CONFIG_SIBLING_ENDPOINTS
        }
        for key in unresolved:
            stack_name = field_to_stack.get(key)
            if stack_name and _stack_exists(region, stack_name):
                failures.append(
                    f"_unresolved_endpoints lists {key!r} but stack {stack_name!r} "
                    f"EXISTS in {region}. The endpoint-race fix-up never ran. "
                    f"Run `make regenerate-runtime-config DEPLOYMENT_STAGE={stage} "
                    f"AWS_REGION={region}`."
                )

    # Best-effort timestamp sanity check (non-fatal if missing for backward compat).
    resolved_at = body.get("_resolved_at")
    if resolved_at is not None:
        if not isinstance(resolved_at, str) or len(resolved_at) < 19:
            failures.append(
                f"_resolved_at={resolved_at!r} is not an ISO-8601-shaped string"
            )

    assert not failures, (
        "runtimeConfig.json endpoint integrity violations:\n  - "
        + "\n  - ".join(failures)
    )
