...
- **Trip Detection**: Automatic trip start/end detection (ignition signal in FWE mode, lifecycle events in MQTT Direct mode). Cross-OEM canonical-event passthrough added in v0.2.0 — TripProcessor accepts both FWE-shape and OEM canonical inputs.
...
│   ├── EventDrivenTelemetryProcessor.java # Fan-out to trips/safety/maintenance topics + Redis LKS
...
│   ├── TripProcessor.java           # Trip analysis (FWE-shape + OEM canonical-event passthrough)
