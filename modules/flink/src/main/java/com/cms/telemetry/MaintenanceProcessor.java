...
public class MaintenanceProcessor {
    ...
    public static void main(String[] args) throws Exception {
        ...
        LOG.error("🔧 Configuration: jobName={}, tableName={}, bootstrapServers={}, awsRegion={}", jobName, tableName, bootstrapServers, awsRegion);
    ...
    public static class MaintenanceHandler implements FlatMapFunction<String, String> {
            ...
            try {
                ...
                if (stateOn && !hasDtcClear && hasDtcCode) {
                    ...
                    if (activeDtcKeys.contains(dedupKey)) {
                        LOG.error("⏭️ Dedup: skipping duplicate vha_diagnostic ACTIVE for key={}", dedupKey);
                        return;
                    }
                    ...
                    String dedupKey = vehicleId + "|" + indicator + "||"
                            + symptomKey + "|" + customerActionKey;
                    if (activeDtcKeys.contains(dedupKey)) {
                        LOG.error("⏭️ Dedup: skipping duplicate vha_diagnostic ACTIVE_NO_DTC for key={}", dedupKey);
                        return;
                    }
                    activeDtcKeys.add(dedupKey);
                    if (activeDtcKeys.size() > 5000) activeDtcKeys.clear();
        ...
        private void writeDtcHistoryRow(
            ...
            try {
                ...
                getDynamoDbClient().putItem(PutItemRequest.builder()
                        ...
                        .item(item)
                        .build());
                LOG.error("🟢 OEM1 dtc-history row written: status={} code={} vehicle={}", status, dtcCode, vehicleId);
            } catch (Exception e) {
            ...
            try {
                ...
                for (Map<String, AttributeValue> row : qresp.items()) {
                    ...
                    getDynamoDbClient().updateItem(UpdateItemRequest.builder()
                            ...
                            .build());
                    LOG.error("🔄 dtc-history updated: {} → {} vehicle={} indicator={}", statusAttr.s(), toStatus, vehicleId, indicator);
                }
            } catch (Exception e) {
        ...
        private void storeActiveDtc(String vehicleId, MaintenanceAlert alert, long currentTime, String odometer) {
            try {
                ...
                LOG.error("🟢 Active DTC emitted: code={} for vehicle={} (dtcId={})",
        ...
        private void emitDtcPendingAction(String vehicleId, String vin,
            ...
            try {
                ...
                getDynamoDbClient().putItem(PutItemRequest.builder()
                        ...
                        .item(item)
                        .build());
                LOG.error("📬 DTC pending-action emitted: code={} vehicle={} "
        ...
        private DynamoDbClient getDynamoDbClient() {
            if (dynamoDbClient == null) {
                ...
                LOG.error("🌍 DynamoDB client region resolved to: {}", region);
