...
public class SafetyProcessor {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String tripsTable = params.get("trips.table.name", null);
        ...
        stream.flatMap(new CatalogDrivenSafetyHandler(safetyTable, eventCatalogTable, signalCatalogTable, regionStr, tripsTable));
    ...
    public static class CatalogDrivenSafetyHandler extends RichFlatMapFunction<String, String> {
        private final String safetyTable, eventCatalogTable, signalCatalogTable, regionStr, tripsTable;
        ...
        // Trip cache — mirrors FWTelemetryProcessor.TRIP_CACHE
        ...
        // Last-fix cache — maps tripId → LastFixCacheEntry (lat/lng from trips table)
        private static final Map<String, LastFixCacheEntry> LAST_FIX_CACHE = new ConcurrentHashMap<>();

        private transient boolean safetyLocationFallbackEnabled = true;

        public CatalogDrivenSafetyHandler(String safetyTable, String eventCatalogTable, String signalCatalogTable, String regionStr) {
            this(safetyTable, eventCatalogTable, signalCatalogTable, regionStr, null);
        }

        public CatalogDrivenSafetyHandler(String safetyTable, String eventCatalogTable, String signalCatalogTable, String regionStr, String tripsTable) {
        ...
        public void open(Configuration parameters) {
            ...
            processedCount = 0;
            eventsGenerated = 0;
            safetyLocationFallbackEnabled = TripProcessor.loadConfig().safetyLocationFallbackEnabled;
        ...
        public void flatMap(String json, Collector<String> out) {
            ...
            try {
                ...
                // Canonical-event passthrough — BEFORE rule-based eval to prevent double-counting
                String cmsEventType = extractValue(json, "cms_event_type");
                if (cmsEventType != null && cmsEventType.startsWith("cms.")) {
                    String safetyEventId = "safety." + cmsEventType.substring("cms.".length());
                    EventRule rule = lookupRuleByCanonicalEventType(safetyEventId);
                    if (rule != null) {
                        if (!isCoolingDown(vehicleId, rule.eventId)) {
                            storeSafetyEvent(json, rule, vehicleId, "cloud-canonical", null);
                            eventsGenerated++;
                        }
                    } else {
                        CW.warn("Canonical safety event %s has no matching rule in catalog", cmsEventType);
                    }
        ...
        /** Looks up an EventRule by its event_id (e.g. "safety.harsh_acceleration"). Returns null if not found. */
        EventRule lookupRuleByCanonicalEventType(String eventId) {
            if (rules == null) return null;
            for (EventRule rule : rules) {
                if (eventId.equals(rule.eventId)) return rule;
            }
            return null;
        }
        ...
        private void storeSafetyEvent(String json, EventRule rule, String vehicleId, String detection, String campaignSyncId) {
            try {
                ...
                if (tripId == null) tripId = resolveActiveTrip(vehicleId);
                ...
                // Location fallback: when lat or lng is absent, stamp the active trip's last-known position
        ...
        // === Trip resolution — mirrors FWTelemetryProcessor.resolveActiveTrip ===
        String resolveActiveTrip(String vehicleId) {
            ...
            try {
                ...
                CW.warn("Trip lookup failed for %s: %s", vehicleId, e.getMessage());
        ...
        /**
         * Returns [lat, lng] from the trip row's lastFixLat/lastFixLng, or null if unavailable.
         * Results are cached with a 60s TTL (same as resolveActiveTrip).
         */
        private double[] getActiveTripLastFix(String tripId, String tripsTable) {
            LastFixCacheEntry cached = LAST_FIX_CACHE.get(tripId);
            if (cached != null && !cached.isExpired()) return cached.fix;
            try {
                QueryResponse resp = ddb.query(QueryRequest.builder()
                        .tableName(tripsTable)
                        .keyConditionExpression("tripId = :t")
                        .expressionAttributeValues(Map.of(":t", AttributeValue.builder().s(tripId).build()))
                        .projectionExpression("tripId, lastFixLat, lastFixLng")
                        .build());
                if (!resp.items().isEmpty()) {
                    Map<String, AttributeValue> row = resp.items().get(0);
                    AttributeValue fixLat = row.get("lastFixLat");
                    AttributeValue fixLng = row.get("lastFixLng");
                    if (fixLat != null && fixLng != null) {
                        double[] fix = new double[]{ Double.parseDouble(fixLat.n()), Double.parseDouble(fixLng.n()) };
                        LAST_FIX_CACHE.put(tripId, new LastFixCacheEntry(fix));
                        return fix;
                    }
                }
                LAST_FIX_CACHE.put(tripId, new LastFixCacheEntry(null));
                return null;
            } catch (Exception e) {
                CW.warn("getActiveTripLastFix failed for tripId=%s: %s", tripId, e.getMessage());
                return null;
            }
        }

        private static class LastFixCacheEntry {
            final double[] fix; // null = no position available
            final long createdAt;
            LastFixCacheEntry(double[] fix) { this.fix = fix; this.createdAt = System.currentTimeMillis(); }
            boolean isExpired() { return System.currentTimeMillis() - createdAt > TRIP_CACHE_TTL_MS; }
        }
        ...
        private List<EventRule> loadCatalogs() {
            ...
            try {
                ...
                for (Map<String, AttributeValue> item : evtResp.items()) {
                    ...
                    if (!rule.jsonFields.isEmpty() || rule.compositeConditions != null || "canonical".equals(rule.conditionType)) {
                        loaded.add(rule);
                    }
