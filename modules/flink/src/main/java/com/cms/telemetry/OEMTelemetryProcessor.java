...
public class OEMTelemetryProcessor {
    ...
    static String extractVehicleId(com.fasterxml.jackson.databind.JsonNode root,
        ...
        // 2026-06-11 (issue oem1-kafka-path-skips-auto-register): self-describing
        // messages carry the real VIN at the root. Prefer it over resolving the
        // shard_key via the vehicles-table deviceToVehicleResolver. The resolver
        // path below remains the defensive fallback for messages without `vin`
        // (legacy in-flight events during deploy, or future feed-format changes).
        // We deliberately do NOT log on the success path — at OEM1 staging volume
        // that would 100x the log rate; only the fallback path logs (see resolver).
        com.fasterxml.jackson.databind.JsonNode vinNode = root.path("vin");
        if (!vinNode.isMissingNode() && !vinNode.isNull()) {
            String vin = vinNode.asText("");
            if (!vin.isEmpty() && !vin.startsWith("aui:")) {
                return vin;
            }
        }
