...
class GRPCConsumer:
    ...
    def _handle_metric(self, vehicle_id: str, feed_event, metric):
        ...
        message_dict = {
            'oem_source': 'ford',  # OEM identifier for transform processor
            'reference': feed_event.reference,
            'timestamp': feed_event.timestamp.ToJsonString(),
            'shardKey': feed_event.shard_key,
            'assetInfo': MessageToDict(feed_event.asset_info),
            'typedData': {
                '@type': 'type.googleapis.com/autonomic.ext.telemetry.Metric',
                **MessageToDict(metric)
            }
        }
        
        # Write to Kafka
        self.kafka_writer.write_message(message_dict, vin=vehicle_id)
