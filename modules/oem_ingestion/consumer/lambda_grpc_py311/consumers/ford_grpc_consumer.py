...
class GRPCConsumer:
    ...
    def _handle_metric(self, vehicle_id: str, feed_event, metric):
        ...
        try:
            ...
            try:
                metric_json = json.loads(MessageToJson(metric, preserving_proto_field_name=True, including_default_value_fields=False))
            except:
                metric_json = {'error': 'Failed to serialize metric'}
                print(f"⚠ Skipping metric with serialization error")
    ...
    def _handle_event(self, vehicle_id: str, feed_event, event):
        ...
        try:
            ...
            try:
                event_json = json.loads(MessageToJson(event, preserving_proto_field_name=True, including_default_value_fields=False))
            except:
                event_json = {'error': 'Failed to serialize event'}
                print(f"⚠ Skipping event with serialization error")
