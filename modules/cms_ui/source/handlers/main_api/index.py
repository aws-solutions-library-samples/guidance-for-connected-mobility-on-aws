        ...
        if path == '/api/v1/fleet-health' and method == 'GET':
            try:
                ...
                if len(warranty) == 0 and len(service) == 0:
                    ...
                    # ── Cost extraction: tolerate mixed shapes ─────────────
                    # The `cost` attribute on service_history items comes in
                    # two shapes in prod (86 of 200 sampled items are the
                    # structured shape):
                    #   (a) plain number:    Decimal("474.38")
                    #   (b) structured map:  {'laborCost': ..., 'partsCost':
                    #                         ..., 'taxCost': ...,
                    #                         'totalCost': ..., 'currency': 'USD'}
                    # Prior to 2026-07-16, `float(s.get('cost', 0))` blew up
                    # with TypeError on shape (b), producing a 500 that the
                    # outer except-clause swallowed to the response body
                    # only (no CloudWatch signal). Issue
                    # 2026-07-16-prod-fleet-health-500.
                    def _extract_cost(rec):
                        c = rec.get('cost', 0)
                        if isinstance(c, dict):
                            # Prefer totalCost; fall back to summing parts if absent.
                            tc = c.get('totalCost')
                            if tc is not None:
                                try:
                                    return float(tc)
                                    ...
                                    return 0.0
                            parts_sum = 0.0
                            for k in ('laborCost', 'partsCost', 'taxCost'):
                                v = c.get(k, 0)
                                try:
                                    parts_sum += float(v)
                                except (TypeError, ValueError):
                    ...
                    total_cost = sum(_extract_cost(s) for s in service)
                    warr_cov = 0.0
                    for s in service:
                        wc = s.get('warrantyCoverage', 0)
                        try:
                            warr_cov += float(wc) if not isinstance(wc, dict) else 0.0
                        except (TypeError, ValueError):
                            pass
                ...
                # Diagnostic logging (issue 2026-07-16-prod-fleet-health-500):
                # the fleet-health outer-except used to swallow the exception
                # to the client response body only, leaving CloudWatch blind.
                # We now log the exception TYPE, message, and full traceback
                # so live 500s can be diagnosed without needing to capture
                # the client-side response body. Print goes to stdout →
                # CloudWatch stream automatically; no logger.setup needed at
                # module load. Response shape unchanged for UI compatibility.
                import traceback as _tb
                print(f"[FLEET-HEALTH][500] {type(e).__name__}: {e}\n{_tb.format_exc()}")
                return {
                    'statusCode': 500,
                    'headers': cors_headers,
                    'body': json.dumps({'error': str(e)})
