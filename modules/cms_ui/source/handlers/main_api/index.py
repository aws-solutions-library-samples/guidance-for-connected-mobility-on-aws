...
def _approve_dtc_action_followups(action_id, vehicle_id, vin, dtc_id, dtc_code,
    ...
    # ── Look up the DTC's human-readable description so the service
    #    row carries it as top-level `description`. This is what the
    #    iOS "Upcoming Service" card displays, and without it the card
    #    falls back to the raw enum "DIAGNOSTIC_REPAIR" which looks
    #    like a schema leak. Best-effort — failure falls back to the
    #    system + dtc_code summary that existed before.
    dtc_human_desc = None
    try:
        dtc_table = dynamodb.Table(f'cms-{stage}-storage-dtc-history')
        # vehicleId is HASH; dtcId was the primary anchor but not the
        # key, so we Query on vehicleId and filter by dtcId. Bounded
        # to 50 rows since we only need one match.
    ...
    # ── Severity → triage priority. Mirrors the voice-triage booking
    #    writer's priority scale so the iOS upcoming-service card sees
    #    the same P0/P1/... badge regardless of which path created the
    #    row.
    ...
    # Human-readable description: prefer the DTC's own description if
    # we successfully looked it up (e.g. "Engine coolant critically
    # overheated") which is what the driver actually cares about.
    # Otherwise fall back to the generic subsystem summary.
    human_description = (
    ...
    # ── Write a service-history row ─────────────────────────────────────
    # Shape mirrors what POST /api/v1/service-history expects (see handler
    # below). Keep fields in sync if that handler's schema evolves.
    service_id = f"SVC-{dtc_id[:8]}-{int(time.time())}"
    service_record = {
        'vehicleId': vehicle_id,
        'serviceDate': resolved_at_iso.split('T')[0],  # YYYY-MM-DD
        'serviceType': 'DIAGNOSTIC_REPAIR',
        'serviceId': service_id,
        # Status uses lowercase "scheduled" to match book.py and the iOS
        # read path. The table has a mix of historical "SCHEDULED" and
        # "scheduled" rows; we write lowercase going forward and the
        # iOS Lambda's FilterExpression tolerates both for backward
        # compat. Normalised 2026-05-05.
        'status': 'scheduled',
        # Top-level fields matching VSA_VOICE_TRIAGE's row shape so iOS's
        # upcoming-service card renders operator-approved DTC services
        # identically to voice-triage bookings. Added 2026-05-05 after
        # the card was showing "DIAGNOSTIC_REPAIR" in raw enum form.
        ...
        # Backlinks so operators can trace "this service row came from that
        # DTC, which came from that action".
    ...
    # ── Clear the DTC ───────────────────────────────────────────────────
    # dtc-history uses (vehicleId, timestamp) as the composite key.  We have
    # dtcId but not timestamp, so query by vehicleId + filter by dtcId to
    # recover the timestamp before updating.
    try:
        dtc_table = dynamodb.Table(
            os.environ.get('DTC_HISTORY_TABLE_NAME', f'cms-{stage}-storage-dtc-history')
        )
        # Paginate newest-first — the DTC we're clearing was just emitted,
        # so it's almost certainly in the first page. But VEH-0025 has
        # hundreds of historical rows and FilterExpression applies AFTER
        # the Limit cut, so a too-small Limit would miss the match. Be
        # generous; these queries are cheap and bounded to a handful of
        # pages by the age of the DTC.
        items = []
        kwargs = {
            'KeyConditionExpression': 'vehicleId = :v',
            'FilterExpression': 'dtcId = :d',
        ...
        # Defensive pagination: stop after 5 pages even if no match
        # (that'd be 2500 rows checked; if our dtc isn't there, something
        # weirder is going on and we shouldn't spin forever).
        for _ in range(5):
        ...
        if items:
            # Pick the most-recent matching row (should only be one, but
            # defensive against dedup-set bugs writing duplicates).
            ...
            dtc_table.update_item(
                ...
                UpdateExpression='SET #s = :s, clearedDate = :c, relatedServiceId = :r',
    ...
    try:
        ...
        if path.startswith('/api/v1/vehicles/') and '/dtcs/' in path and method == 'PATCH':
            ...
            try:
                ...
                dtc_table.update_item(
                    ...
                    UpdateExpression='SET #s = :s, clearedDate = :c, relatedServiceId = :r, clearedBy = :cb',
                    ExpressionAttributeNames={'#s': 'status'},
                    ExpressionAttributeValues={
                        ':s': 'CLEARED',
                        ':c': now_iso,
            ...
            try:
                ...
                def fetch_fleet(vehicle):
                    ...
                    try:
                        ...
                        return resp.get('Item', {}).get('name', 'Unknown Fleet')
                    except: return 'Unknown Fleet'
                ...
                def fetch_safety():
                    try:
                        ...
                        for a in items:
                            ...
                            if 'lat' in a and 'latitude' not in a: a['latitude'] = a['lat']
                            if 'lng' in a and 'longitude' not in a: a['longitude'] = a['lng']
                        return items
                    except: return []
                ...
                fleet_name = fleet_future.result()
        ...
        if (path == '/api/v1/vehicles/locations' or path == '//api/v1/vehicles/locations') and method == 'GET':
            try:
                ...
                # Overlay live locations from Redis geo index
                try:
                    rc = _get_redis()
                    if rc:
                        geo_results = rc.geosearch("vehicle:locations", 0, 0, 20000)  # global
                        if geo_results:
                            geo_map = {g["vehicleId"]: g for g in geo_results}
                            for vl in vehicle_locations:
                                geo = geo_map.get(vl["vehicleId"])
                                if geo:
                                    vl["lat"] = geo["lat"]
                                    vl["lng"] = geo["lng"]
                                    vl["connectionStatus"] = "connected"
                                    # Get speed from signals hash
                                    meta = rc.hgetall(f"vehicle:{vl['vehicleId']}:meta")
                                    if meta.get("lastSeenAt"):
                                        try: vl["lastUpdate"] = int(float(meta["lastSeenAt"]) / 1000)
                                        except ValueError: pass
                except Exception as redis_err:
                    print(f"Redis locations overlay skipped: {redis_err}")
        ...
        if path == '/api/v1/documents' and method == 'GET':
            try:
                ...
                bucket = os.environ.get('VFO_KB_BUCKET', 'cms-prod-vfo-knowledge-base')
