...
async function main() {
  ...
  const valid = await isValidRequest(policy, kpid, exp, sig, rfp, nonceHash);
  if (valid) {
    ...
    // If CFS already returned signed params but validation still failed, we're in
    // an unresolvable loop (e.g. stale RFP after cookie expiry on soft refresh).
    // Clear all CFS cookies and redirect to the original target URL — a clean load
    // of the target will 403, run this script fresh with no stale state, and re-auth.
    if (encodedTargetUrl) {
      ['CloudFront-Policy','CloudFront-Key-Pair-Id','CloudFront-Signature','CloudFront-Expiration','CloudFront-RFP']
        .forEach(function(name) { Cookies.remove(name, { secure: true, sameSite: 'Lax' }); });
