...
// Mock client implementation - no longer using Smithy-generated client
export interface FleetManagementClientConfig {
  endpoint: string;
  apiKey?: { apiKey: string };
}

export class MockFleetManagementClient {
  ...
  async send(command: any): Promise<any> {
    // Mock implementation
    console.log('Mock client send:', command);
    return Promise.resolve({});
  }
}
  private SIGNAL_CATALOGS: SignalCatalogItem[];
  private SIGNAL_CATALOG_NODES: Record<string, any[]>;
  ...
  constructor(config: FleetManagementClientConfig) {
    ...
    this.SIGNAL_CATALOG_NODES = SIGNAL_CATALOG_NODES;
  ...
  async send<T extends Command<any, any, any>>(command: T): Promise<any> {
    ...
    } else if (command instanceof ListSignalCatalogNodesCommand) {
      const catalogName = command.input.name;
      return Promise.resolve({ 
        name: catalogName,
        nodes: this.SIGNAL_CATALOG_NODES[catalogName] || [] 
      });
    ...
    } else if (command instanceof UpdateUserPreferencesCommand) {
      console.log("Mock client: Handling UpdateUserPreferencesCommand", command.input);
      // Just return a success response, the actual state is managed in the UserContext
      return Promise.resolve({ 
