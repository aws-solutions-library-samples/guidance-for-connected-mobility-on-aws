...
interface Driver {
  ...
  // Status enum widened 2026-05-29 from {'active', 'inactive'} to
  // {'active', 'on_leave', 'terminated'} to match the seed-script and
  // server-side API contract (spec
  // 2026-05-29-staging-drivers-simulator-cognito-parity Decision 6). The
  // 2026-05-28 defensive guard in the status cell-renderer (treats falsy
  // values as an em-dash) remains as belt-and-suspenders for stale
  // cached responses.
...
const DriversView: React.FC = () => {
  ...
  const fetchDrivers = async () => {
    try {
      setIsLoading(true);
      const response = await authFetch(`${getRuntimeConfig().apiEndpoint}api/v1/drivers?_t=${Date.now()}`);
  ...
  const fetchFleets = async () => {
    try {
      const response = await authFetch(`${getRuntimeConfig().apiEndpoint}api/v1/fleets?_t=${Date.now()}`);
      const data = await response.json();
  ...
  const handleCreate = async () => {
    try {
      const response = await authFetch(`${getRuntimeConfig().apiEndpoint}api/v1/drivers`, {
        method: 'POST',
        headers: {
  ...
  const handleEdit = async () => {
    ...
    try {
      const response = await authFetch(`${getRuntimeConfig().apiEndpoint}api/v1/drivers/${editingDriver.driverId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleDelete = async () => {
    try {
      for (const driver of selectedItems) {
        const response = await authFetch(`${getRuntimeConfig().apiEndpoint}api/v1/drivers/${driver.driverId}`, {
          method: 'DELETE'
        });
  ...
  const columnDefinitions = [
    ...
    },
    {
      ...
      header: 'Status',
      cell: (item: Driver) => {
        // Defensive: some seeded drivers may lack a `status` field. Render
        // an em-dash placeholder rather than crashing the table on charAt().
        const status = item.status;
        if (!status) {
          return <StatusIndicator type="stopped">—</StatusIndicator>;
        }
        // Status enum widened 2026-05-29 to {active, on_leave, terminated}
        // (spec 2026-05-29-staging-drivers-simulator-cognito-parity Decision 6).
        // Render mapping:
        //   active     → green success indicator, "Active"
        //   on_leave   → yellow in-progress indicator, "On leave"
        //   terminated → red error indicator, "Terminated"
        //   <other>    → grey stopped indicator, raw value (forward-compat
        //                for any future enum value the backend introduces)
        const indicatorType: 'success' | 'in-progress' | 'error' | 'stopped' =
          status === 'active' ? 'success'
          : status === 'on_leave' ? 'in-progress'
  ...
  const assignedCount = drivers.filter(d => d.fleetId).length;
  ...
  return (
    <SpaceBetween size="l">
      ...
      <Grid gridDefinition={[{ colspan: 3 }, { colspan: 3 }, { colspan: 3 }, { colspan: 3 }]}>
        <Container header={<Header variant="h2">Total Drivers</Header>}>
          <Box variant="h1" color="text-status-info">{drivers.length}</Box>
          <div><Box variant="small" color="text-body-secondary">Registered in system</Box></div>
        </Container>
        <Container header={<Header variant="h2">Active</Header>}>
          <Box variant="h1" color="text-status-success">{activeCount}</Box>
          <div><Box variant="small" color="text-body-secondary">Currently on duty</Box></div>
          <div><Box variant="small" color="text-status-info">{drivers.length > 0 ? Math.round(activeCount / drivers.length * 100) : 0}% of total</Box></div>
        </Container>
        <Container header={<Header variant="h2">Fleet Assigned</Header>}>
          <Box variant="h1" color="text-status-info">{assignedCount}</Box>
          <div><Box variant="small" color="text-body-secondary">Assigned to a fleet</Box></div>
          <div><Box variant="small" color="text-status-warning">{drivers.length - assignedCount} unassigned</Box></div>
        </Container>
        <Container header={<Header variant="h2">License Expiring</Header>}>
          <Box variant="h1" color="text-status-warning">2</Box>
          <div><Box variant="small" color="text-body-secondary">Within 90 days</Box></div>
          <div><Box variant="small" color="text-status-error">1 expired</Box></div>
        </Container>
      </Grid>
        ...
        <Form>
          <SpaceBetween size="m">
            ...
            <FormField label="Status">
              {/* Status enum widened 2026-05-29: {active, on_leave, terminated}.
                  Form mirrors the seed-script + server-side enum. */}
              <Select
                selectedOption={{
                  label:
                    formData.status === 'active' ? 'Active'
                    : formData.status === 'on_leave' ? 'On leave'
                    : formData.status === 'terminated' ? 'Terminated'
                    : formData.status,
                  value: formData.status,
                }}
                onChange={({ detail }) => setFormData(prev => ({
                  ...prev,
                  status: detail.selectedOption?.value as 'active' | 'on_leave' | 'terminated',
                }))}
                options={[
