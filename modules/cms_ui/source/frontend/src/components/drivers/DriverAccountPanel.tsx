...
/**
 * DriverAccountPanel — VSA Cognito account status + lock/unlock controls
 * for a given driver.
 *
 * Sits on the driver detail page to close the loop between the drivers
 * table (CMS operator-facing) and the VSA Cognito pool (iOS-app-facing).
  ...
  return (
    <Container
      header={
        <Header
          variant="h2"
          description="Driver-facing Cognito account for the iOS app (VSA pool)"
          actions={
    ...
    >
      ...
      {loading ? (
        ...
        <Alert type="warning" header="No iOS app account provisioned">
          Driver <strong>{driverId}</strong> ({status.email}) has no VSA Cognito user.
          Run{' '}
