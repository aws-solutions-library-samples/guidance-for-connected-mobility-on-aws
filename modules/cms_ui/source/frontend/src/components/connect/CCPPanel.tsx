...
export const CCPPanel: React.FC<CCPPanelProps> = ({
  ...
  useEffect(() => {
    ...
    // `connect.core` is a GLOBAL singleton; the iframe initCCP creates
    // survives React remounts (a route change recreates this component and
    // resets the per-instance `initializedRef`). Re-calling initCCP then
    // throws "Attempted to call initCCP when an iframe generated by initCCP
    // already exists". If the SDK is already initialized, skip initCCP and
    // just re-wire this instance's subscribers so handlers stay current.
    // (issue 2026-06-22-cms-ccp-double-init)
    if (window.connect.core && window.connect.core.initialized) {
