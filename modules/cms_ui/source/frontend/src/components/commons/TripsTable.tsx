...
export const TripsTable: React.FC<TripsTableProps> = ({
  ...
  const [preferences, setPreferences] = useState({
    pageSize: 10,
    visibleContent: ['vehicleId', 'driverId', 'startTime', 'duration', 'distance', 'avgSpeed', 'driverScore', 'status', 'actions']
  });
  ...
  const columnDefinitions = [
    ...
    ...(showDriverColumn ? [{
      ...
      header: 'Driver',
      cell: (item: Trip) => {
        if (item.driverName && !item.driverName.startsWith('DRV-') && !item.driverName.startsWith('DRIVER-')) {
    ...
    },
    {
      ...
      header: 'Status',
      cell: (item: Trip) => {
        const isActive = !item.endTime || item.status === 'ACTIVE' || item.status === 'active' || item.status === 'in_progress';
        return (
          <StatusIndicator type={isActive ? 'in-progress' : 'success'}>
  ...
  // Filter and sort trips based on search text
