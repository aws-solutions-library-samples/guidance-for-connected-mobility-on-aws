...
const DecoderManifestViewer: React.FC = () => {
  ...
  const [pairedModelName, setPairedModelName] = useState<string | null>(null);
  ...
  const viewManifest = (manifest: DecoderManifest) => {
    ...
    Promise.all([
      ...
      authFetch(`${API()}/model-manifests`).then(r => r.json()),
    ]).then(([sigData, ifData, modelData]) => {
      setSignals(sigData.signals || []);
      setInterfaces(ifData.networkInterfaces || []);
      const paired = (modelData.modelManifests || []).find(
        (m: any) => m.decoderManifestRef === manifest.decoderManifestName
      );
      setPairedModelName(paired?.modelManifestName ?? null);
    ...
    <>
      ...
      <Modal visible={showDetail} onDismiss={() => setShowDetail(false)}
        ...
        <SpaceBetween size="l">
          {pairedModelName && (
            <Box>
              <Box variant="awsui-key-label">Paired vehicle model</Box>
              <Badge color="green">{pairedModelName}</Badge>
            </Box>
          )}
