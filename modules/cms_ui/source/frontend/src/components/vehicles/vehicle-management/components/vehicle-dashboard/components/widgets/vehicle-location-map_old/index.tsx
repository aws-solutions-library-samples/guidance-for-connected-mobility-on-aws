...
import { VehicleItem } from "@/api/fleet-management-client";
...
export default function WidgetContent() {
  ...
  useEffect(() => {
    // Check if mapAuth configuration is available
    if (runtimeConfig.mapAuth && 
        runtimeConfig.mapAuth.identityPoolId && 
        runtimeConfig.mapAuth.identityPoolId !== 'test' &&
        runtimeConfig.mapAuth.identityPoolClient) {
      ...
      withIdentityPoolId(runtimeConfig.mapAuth.identityPoolId, {
        ...
        console.warn('Failed to initialize map authentication:', error);
        setAuthHelper(null);
      });
    } else {
      console.warn('Map authentication not configured, skipping map initialization');
      setAuthHelper(null);
    }
  ...
  return authHelper === undefined ? (
    ...
    <div style={{ padding: '20px', textAlign: 'center', color: '#666' }}>
      Map not available - authentication not configured
    </div>
  ) : (
    <Map
      id="map-view"
      ...
      mapStyle={
        runtimeConfig.mapAuth && runtimeConfig.mapAuth.mapName
          ? `https://maps.geo.${runtimeConfig.awsRegion}.amazonaws.com/maps/v0/maps/${runtimeConfig.mapAuth.mapName}/style-descriptor?color-scheme=${mapTheme}`
          : undefined
      }
      maxZoom={16}
      dragPan={false}
