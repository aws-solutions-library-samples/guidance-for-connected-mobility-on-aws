...
export function TripMap({ route, startLocation, endLocation, safetyEvents = [], height = '400px', isActive = false, vehicleType = 'Sedan', showStartEndMarkers = false }: TripMapProps) {
  ...
  // Don't render a map centered on an arbitrary default when there's nothing to show
  const hasRoute = Array.isArray(route) && route.length > 0;
  const hasLocation = startLocation?.lat != null;
  if (!hasRoute && !hasLocation) return null;
  const [mapConfig, setMapConfig] = useState<MapConfig | null>(null);
  // Map re-fetch race fix — mirrors commit `2f5c435` on FleetVehicleMapView.
  // Empty-dep mount fires before the idToken lands after fresh login →
  // getMapConfiguration returns OSM fallback → map stuck on OSM until reload.
  // Depend on `auth.user` so we re-fetch once auth hydrates.
  const auth = useAuth();
  ...
  const getInitialViewState = () => {
    ...
    return {
      longitude: startLocation?.lng || startLocation?.lon || -98.5,
      latitude: startLocation?.lat || 39.5,
      zoom: 14
    };
  };
