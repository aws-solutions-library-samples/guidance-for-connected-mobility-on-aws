...
export function TripMap({ route, startLocation, endLocation, safetyEvents = [], height = '400px', isActive = false, vehicleType = 'Sedan', showStartEndMarkers = false }: TripMapProps) {
  ...
  useEffect(() => {
    if (mapRef.current && Array.isArray(route) && route.length > 0) {
      // Small delay to ensure map is fully loaded
      setTimeout(() => {
        try {
          console.log('Route data:', route.slice(0, 3)); // Log first 3 points for debugging
          
          const lngs = route.map(point => parseFloat(point.lng)).filter(lng => !isNaN(lng));
          const lats = route.map(point => parseFloat(point.lat)).filter(lat => !isNaN(lat));
          
          console.log('Valid coordinates:', { lngs: lngs.slice(0, 3), lats: lats.slice(0, 3) });
