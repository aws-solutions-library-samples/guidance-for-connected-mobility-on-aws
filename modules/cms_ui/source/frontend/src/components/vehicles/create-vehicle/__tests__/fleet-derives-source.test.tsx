...
function renderForm(fleets: FleetItem[]) {
  // The form's FleetPicker now reads /api/v1/fleets via authFetch (rev 4 —
  // post-2026-06-15 fix); we mock authFetch per-test to return the fleets
  // payload. The legacy ApiContext.client.send mock is retained for any
  // unrelated callers in the form tree but no longer drives FleetPicker.
  authFetchMock.mockReset();
  authFetchMock.mockImplementation(() => FLEETS_OK(fleets as any[]));
