...
const TripSimulatorModal: React.FC<TripSimulatorModalProps> = ({ visible, vehicleId, onDismiss, onStarted }) => {
  ...
  useEffect(() => {
    const fetchCatalog = async () => {
      ...
      try {
        const resp = await fetch(`${API()}/api/v1/event-catalog`);
        if (resp.ok) {
  ...
  return (
    <Modal visible={visible} onDismiss={onDismiss} header="Trip Simulator"
      ...
      <SpaceBetween size="m">
        <Alert type="info">
          Simulate a trip for <strong>{vehicleId}</strong>. Select events from the catalog to test during this trip.
        </Alert>
