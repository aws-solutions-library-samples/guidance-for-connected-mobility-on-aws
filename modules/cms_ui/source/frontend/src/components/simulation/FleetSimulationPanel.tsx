...
export default function FleetSimulationPanel() {
  ...
  const [fleetNames, setFleetNames] = useState<Record<string, string>>({});
  ...
  useEffect(() => {
    (async () => {
      try {
        const resp = await authFetch(`${apiEndpoint}/api/v1/event-catalog`);
        if (resp.ok) {
  ...
  const fetchVehicles = useCallback(async () => {
    try {
      // onboard_only: off-board OEM (cloud-fed, e.g. OEM1) vehicles can't be
      // simulated — their telemetry comes from the external feed, not the sim.
      const res = await authFetch(`${apiEndpoint}/api/v1/vehicles?limit=100&onboard_only=true`);
      if (res.ok) {
        const data = await res.json();
  ...
  const fetchFleets = useCallback(async () => {
    try {
      const res = await authFetch(`${apiEndpoint}/api/v1/fleets`);
      if (res.ok) {
        const data = await res.json();
        const map: Record<string, string> = {};
        for (const f of data.fleets || []) {
          if (f.fleetId) map[f.fleetId] = f.name || f.fleetId;
        }
        setFleetNames(map);
      }
    } catch {}
  ...
  }, [fetchSimulations, fetchVehicles, fetchFleets]);
        ...
        <Modal visible onDismiss={() => setShowVehicleModal(false)} size="max"
          ...
          <Table
            ...
            columnDefinitions={[
              ...
              { id: 'fleet', header: 'Fleet', cell: (v: any) => v.fleetId ? <Badge>{fleetNames[v.fleetId] || v.fleetId}</Badge> : '—' },
