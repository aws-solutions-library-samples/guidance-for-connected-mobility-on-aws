...
const SafetyAlertsPage: React.FC = () => {
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      // ── Event sample (for leaderboards + metrics) ───────────────
      if (eventsResp.status === "fulfilled" && eventsResp.value.ok) {
        const data = await eventsResp.value.json();

        // Normalise severity: DDB stores numeric strings "1"/"2"/"3"
        const normSev = (raw: any): string => {
        ...
        const list: SafetyEvent[] = (data.events || []).map((e: any) => ({
          ...
          // API converts ms→s; normalise back to ms for display
          timestamp: (() => {
        ...
        // Derive metrics from sample when dashboard cache is absent
        // Use API-provided total if available (500-sample may be partial)
        const apiTotal = Number(data.total) || list.length;
        const fromSample = (sev: string) => list.filter(e => e.severity === sev).length;
        // Scale counts proportionally if sample < total
        const scale = apiTotal > list.length && list.length > 0 ? apiTotal / list.length : 1;
        setMetrics(prev => ({
          total: prev.total > 0 ? prev.total : apiTotal,
          critical: prev.critical > 0 ? prev.critical : Math.round(fromSample("CRITICAL") * scale),
          high: prev.high > 0 ? prev.high : Math.round(fromSample("HIGH") * scale),
          medium: prev.medium > 0 ? prev.medium : Math.round(fromSample("MEDIUM") * scale),
          low: prev.low > 0 ? prev.low : Math.round(fromSample("LOW") * scale),
        }));
      } else {
