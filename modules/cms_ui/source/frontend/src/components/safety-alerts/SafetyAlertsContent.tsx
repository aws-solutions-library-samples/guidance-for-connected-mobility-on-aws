  ...
  const fetchSafetyEvents = async (page: number = 1, limit: number = 20, filters?: { fleetId?: string; eventType?: string; timeRange?: string }) => {
    try {
      ...
      const transformedEvents: SafetyEvent[] = apiAlerts.map((alert: any) => {
        // Normalise timestamp → always a number (ms). API returns seconds or ms.
        ...
        if (typeof rawTs === 'number') {
          ...
          tsMs = isNaN(n) ? new Date(rawTs).getTime() : (n > 1e12 ? n : n * 1000);
        }

        // Normalise location — API returns top-level latitude/longitude
        const lat = parseFloat(alert.latitude ?? alert.location?.latitude ?? alert.location?.lat ?? 0);
        const lon = parseFloat(alert.longitude ?? alert.location?.longitude ?? alert.location?.lon ?? 0);
