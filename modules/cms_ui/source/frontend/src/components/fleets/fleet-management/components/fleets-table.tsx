...
const rawColumns = (navigate: (path: string) => void) => [
  {
    id: "name",
    sortingField: "name",
    header: "Fleet Name",
    cell: (item: FleetItem) => (
      <div>
        <Link onFollow={(e) => { e.preventDefault(); navigate(`/fleets/management/${item.id || item.fleetId}`); }}>{item.name}</Link>
      </div>
    ),
    minWidth: 180,
  },
...
export default function FleetsTable({
  ...
  const columnDefinitions = rawColumns(navigate).map((column) => ({
    ...column,
