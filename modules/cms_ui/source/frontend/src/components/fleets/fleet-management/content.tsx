...
export function Content() {
  ...
  const fetchFleet = async (fleetId: string) => {
    ...
    try {
      const response = await authFetch(`${getRuntimeConfig().apiEndpoint}api/v1/fleets/${fleetId}`);
      const data = await response.json();
  ...
  const onDeleteConfirm = async () => {
    ...
    const fleetsDeletePromises = fleetsToDelete.map(async (fleet) => {
      ...
      try {
        const response = await authFetch(`${getRuntimeConfig().apiEndpoint}api/v1/fleets/${fleet.id}`, {
          method: 'DELETE'
        });
  ...
  return (
    <>
      {fleetId && !locationFleet && isLoading ? (
        ...
        <FleetDetailsPage
          fleetId={locationFleet.fleet?.id || locationFleet.fleet?.fleetId || locationFleet.id || fleetId}
