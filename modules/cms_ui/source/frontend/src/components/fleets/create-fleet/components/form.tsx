...
import { FleetSourcePicker, type FleetSourceSelection } from './FleetSourcePicker';
...
export function FormFull({ loadHelpPanelContent, header }: any) {
  ...
  const [selection, setSelection] = useState<FleetSourceSelection>({ telemetrySource: 'on-board' });
  ...
  const createFleet = async () => {
    try {
      ...
      payload.data_source = selection.telemetrySource === 'off-board' ? 'cloud-telemetry' : 'vehicle-telemetry';
      if (selection.telemetrySource === 'off-board' && selection.transformManifestId) {
        payload.transform_manifest_id = selection.transformManifestId;
      }
      if (selection.defaultVehicleModelId) {
        payload.default_vehicle_model_id = selection.defaultVehicleModelId;
      }
  ...
  return (
    <div>
      <BaseForm
        ...
        onCancelClick={() => {
          setData(defaultData);
          setSelection({ telemetrySource: 'on-board' });
          navigate(UI_ROUTES.FLEET_MANAGEMENT);
        }}
