...
const ChargingManagementView: React.FC = () => {
  ...
  useEffect(() => {
    ...
    authFetch(`${base}/api/v1/charging/summary`).then(r => r.json()).then(setSummary).catch(() => {});
    authFetch(`${base}/api/v1/charging/sessions`).then(r => r.json()).then(d => setSessions(d.sessions || [])).catch(() => {});
  }, []);
  ...
  return (
    <SpaceBetween size="l">
      ...
      <Grid gridDefinition={[{ colspan: 3 }, { colspan: 3 }, { colspan: 3 }, { colspan: 3 }]}>
        <Container header={<Header variant="h2">Sessions Today</Header>}>
          <Box variant="h1" color="text-status-info">{summary?.sessionsToday ?? '—'}</Box>
          <Box variant="small" color="text-body-secondary">{summary ? 'Charging sessions started today' : 'No data available'}</Box>
        </Container>

        <Container header={<Header variant="h2">BEV Vehicles</Header>}>
          <Box variant="h1" color="text-status-success">{summary?.bevVehicles ?? '—'}</Box>
          <Box variant="small" color="text-body-secondary">{summary ? 'Vehicles with charging history' : 'No data available'}</Box>
        </Container>

        <Container header={<Header variant="h2">Energy Delivered Today</Header>}>
          <Box variant="h1" color="text-status-info">{summary?.kwhToday != null ? `${summary.kwhToday} kWh` : '—'}</Box>
          <Box variant="small" color="text-body-secondary">{summary ? "Today's total consumption" : 'No data available'}</Box>
        </Container>

        <Container header={<Header variant="h2">Charging Cost MTD</Header>}>
          <Box variant="h1" color="text-status-warning">{summary?.costMTD != null ? `$${summary.costMTD.toFixed(0)}` : '—'}</Box>
          <Box variant="small" color="text-body-secondary">{summary?.totalSessionsMTD != null ? `${summary.totalSessionsMTD} sessions this month` : 'No data available'}</Box>
        </Container>
      </Grid>
