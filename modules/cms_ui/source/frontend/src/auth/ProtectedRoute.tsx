  ...
  if (!auth.isAuthenticated) {
    ...
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', flexDirection: 'column', gap: '16px' }}>
        <Alert statusIconAriaLabel="Info" type="info" header="Authentication Required"
          action={{ children: 'Sign In with Email', onClick: auth.login }}
        >
          Please sign in to access this application.
        </Alert>
        <button
          onClick={() => auth.loginWithFederate?.()}
          style={{ padding: '10px 24px', background: '#232f3e', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '14px' }}
        >
