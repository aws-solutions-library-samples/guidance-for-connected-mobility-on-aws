...
export interface UseRealtimeDataOptions {
  ...
  /** Fleet to subscribe to. Omit/`*` for an admin all-fleet stream. */
  fleetId?: string;
}
...
export function useRealtimeConnection(
  ...
  const { autoConnect = true, reconnectOnError = true, fleetId: optFleetId } = options;

  useEffect(() => {
    const wsEndpoint = getRuntimeConfig().wsEndpoint;
    if (!wsEndpoint) return; // no realtime endpoint configured → callers fall back to polling

    const groups = user?.groups || [];
    const isAdmin = groups.includes('platform-admin');
    const userFleetIds = (user?.fleetIds || '').split(',').map(s => s.trim()).filter(Boolean);
    // platform-admin → all-fleet stream (omit fleetId; backend stores under '*').
    // non-admin → the requested fleet, else their first claimed fleet.
    const fleetId = optFleetId && optFleetId !== '*' ? optFleetId : (isAdmin ? undefined : userFleetIds[0]);
    if (!isAdmin && !fleetId) return; // non-admin with no fleet can't connect → polling fallback

    serviceRef.current = getRealtimeService(wsEndpoint, { tokenProvider: getIdToken, fleetId });
  ...
  }, [apiEndpoint, autoConnect, reconnectOnError, optFleetId, user?.groups, user?.fleetIds, getIdToken]);
