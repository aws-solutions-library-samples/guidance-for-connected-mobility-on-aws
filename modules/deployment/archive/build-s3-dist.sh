...
# Set deployment ID (use provided value or generate timestamp-based ID)
if [ -z "$DEPLOYMENT_ID" ]; then
  export DEPLOYMENT_ID=$(date +"%Y%m%d%H%M%S")
  echo "No DEPLOYMENT_ID provided, using generated ID: $DEPLOYMENT_ID"
else
  echo "Using provided DEPLOYMENT_ID: $DEPLOYMENT_ID"
fi
...
printf "%b[Build] Build Smithy model and components\n%b" "${GREEN}" "${NC}"
...
printf "%b[Build] Build ACDP Assets\n%b" "${GREEN}" "${NC}"

../../../deployment/module-build/build-acdp-assets.sh

printf "%b[Build] Build CDK Assets\n%b" "${GREEN}" "${NC}"
../../../deployment/module-build/build-cdk-assets.sh

#printf "%b[Test] Running Lambda packaging tests\n%b" "${GREEN}" "${NC}"
#cd "$ROOT_DIR" && make test-lambda-packaging
