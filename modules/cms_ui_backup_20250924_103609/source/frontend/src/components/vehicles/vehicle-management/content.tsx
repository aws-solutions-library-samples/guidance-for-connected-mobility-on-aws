...
export function Content() {
  ...
  async function fetchVehicles(page: number = 1, pageSize: number = 25, fleetId?: string): Promise<{ vehicles: VehicleItem[], total: number, pagination: any }> {
  ...
  const handleFleetFilterChange = async (fleetId: string) => {
    ...
    try {
      const result = await fetchVehicles(1, pageSize, fleetId);
      setVehicles(result.vehicles);
      setTotalVehicleCount(result.total);
      setPaginationInfo(result.pagination);
    } catch (err) {
