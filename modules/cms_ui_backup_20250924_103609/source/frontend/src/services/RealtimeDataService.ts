...
export class RealtimeDataService extends EventEmitter {
  ...
  constructor(apiEndpoint: string) {
    ...
    this.wsEndpoint = apiEndpoint.replace('https://', 'wss://').replace('http://', 'ws://') + '/ws';
  }
 ...
 */
export function getRealtimeService(apiEndpoint?: string): RealtimeDataService {
  if (!realtimeService && apiEndpoint) {
    realtimeService = new RealtimeDataService(apiEndpoint);
  }
  if (!realtimeService) {
    throw new Error('RealtimeDataService not initialized. Provide apiEndpoint.');
  }
