...
### Device-VIN binding (telemetry join key)

OEM1 telemetry arrives over the OEM1 cloud feed keyed by an asset/device UUID
(`shard_key` = `aui:asset:<vehicle|device>/<UUID>`), not by VIN. Two pieces of
state bind a device to its VIN, with distinct responsibilities:

- **Canonical join key — VIN in Kafka payload (pipeline-correctness state).**
  The connector extracts the real VIN from the feed's `asset_info` and stamps it
  as a top-level `vin` field on every Way B Kafka message
  (`connector.py:_kafka_raw_payload`). `OEMTelemetryProcessor.extractVehicleId`
  prefers this self-describing VIN, so trips and downstream rows are VIN-keyed at
  the source. This is the authoritative join key; it does not depend on any
  table lookup.

- **`oem1_device_uuid` on the VEHICLE row (operator-visibility state).**
  Populated lazily from telemetry: the Way B run loop calls
  `auto_register.handle_unknown_vin` (throttled to ≤1 DynamoDB write per VIN per
  hour, best-effort) so the vehicles table records the device↔VIN pairing and
  `last_seen_at`. This drives UI affordances such as "device paired with VIN X,
  last seen at Y". It is **not** required for pipeline correctness — if it is
  stale or absent, VIN-keyed trips still flow because the VIN travels in the
  message.

- **`deviceToVehicleResolver` (defensive fallback).** When a message arrives
  without a usable `vin` (legacy in-flight messages during a deploy, or a future
  feed-format change), `OEMTelemetryProcessor` falls back to resolving the
  `shard_key` UUID against the `oem1_device_uuid` column via the
  `aui_asset_resolve` transform. The resolver is retained precisely for this
  edge case; the steady-state path is VIN-in-payload.

> History: prior to the 2026-06-11 fix
> (`issues/2026-06-11-oem1-kafka-path-skips-auto-register/`), the Way B Kafka
> path neither stamped the VIN nor called `handle_unknown_vin`, so the resolver
> was the only join path and resolved only the single vestigially-populated row
> — leaving 47/48 enrolled vehicles producing UUID-keyed orphan trips.
