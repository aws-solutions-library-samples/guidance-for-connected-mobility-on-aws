...
Per processor lifetime, the canonical handler maintains an in-memory `activeDtcKeys` set (5000-bounded) keyed by:

```
vehicleId | indicator | dtc_code | symptom_key | customer_action_key
```

This 5-tuple structurally distinguishes:
...
The dedup is best-effort, not authoritative — a Flink processor restart resets the set. Downstream readers must tolerate occasional duplicate writes within the brief window of a restart.
