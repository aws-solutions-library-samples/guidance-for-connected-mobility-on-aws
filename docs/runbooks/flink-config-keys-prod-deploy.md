...
> **Note for spec `2026-06-17-oem1-trip-route-distinct-position-fixes`**: This spec's deploy is **JAR-only** — no config-key surface change beyond the JAR-bundled `trip-processor-config.json`. The deploy reduces to: (a) push the new JAR (built via `mvn clean package`) to the artifact bucket, (b) call `update-application` with the new code-content version pointer, (c) post-deploy smoke test. No CDK change-set required. Use the out-of-band KDA deploy runbook documented in the spec's Constraints section instead of the change-set process below.
