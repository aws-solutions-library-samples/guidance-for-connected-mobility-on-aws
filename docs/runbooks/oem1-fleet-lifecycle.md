...
- For TC8030 (VIN not in OEM1 ecosystem), verify the VIN is correct and is an enrolled vehicle in OEM1's system. If not, that vehicle cannot be managed via OEM1 and should remain on an on-board (FWE) fleet (`data_source = vehicle-telemetry`).
