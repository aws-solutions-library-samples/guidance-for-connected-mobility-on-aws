...
For the real DLQ sample: `shard_key = "aui:asset:device/<device-uuid>"`.
After `substring_after_last_slash` → `"<device-uuid>"` (a device UUID, NOT a vehicleId).
...
```json
{
  "stringLabel": "aui:event:<tenant-uuid>:custom:vha-diagnostics-processed-event",
  "metrics": [{
