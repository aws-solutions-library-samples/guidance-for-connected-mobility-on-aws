...
- **OEM Cloud-to-Cloud** — Tesla, Ford, Rivian, etc. (OEM-specific JSON/protobuf)
...
OEM API → cms-telemetry-oem (raw OEM JSON)
  → OEMTelemetryProcessor:
      ...
      5. Extract vehicleId from OEM-specific location (e.g., Ford shardKey)
...
```json
{
  ...
  "source_name": "ford",
  "source_format": "json",
  "description": "Ford Pro Telematics → CMS normalization",
...
### 5.1 Ford Pro Telematics (gRPC Streaming)

**Updated:** 2026-03-23 — Based on Ford Feed Service SDK documentation and Data Dictionary v2.

```
Ford TMC Feed Service (gRPC)
  → Ford Connector (ECS Fargate, long-running gRPC client)
      ...
      4. For each FeedEvent:
         a. Decode google.protobuf.Any typed_data → JSON
         b. Attach oem_source: "ford-feed"
         c. Write JSON to MSK: cms-telemetry-oem
  → OEMTelemetryProcessor (Flink) applies ford-feed-transform.json manifest
  → cms-telemetry-preprocessed
```
...
**Ford Feed Service Data Dictionary (key signals):**

| Well Known Signal | Value Type | Unit | CMS `json_field` | Conversion | Notes |
|---|---|---|---|---|---|
| `SPEED` | Speed (speed: Float, detectionType: Enum) | m/s | `speed` | `mps_to_mph` | Extract `speed` from compound type |
| `ODOMETER` | Double | km | `odometer` | `km_to_miles` | |
| `POSITION` | Location (threeDPoint: lat/lng/alt, gpsDimension) | deg/m | `lat`, `lng` | None | Extract from `location.threeDPoint` |
| `HEADING` | HeadingValue (heading: Float, detectionType) | deg | `heading` | None | Extract `heading` from compound type |
| `FUEL_LEVEL` | Double | % | `fuelLevel` | None | Range: -5.2 to 106% (accounts for reserve) |
| `ENGINE_SPEED` | Integer | rpm | `engineRPM` | None | |
| `ENGINE_COOLANT_TEMP` | Double | °C | `engineTemp` | `C_to_F` | |
| `ENGINE_OIL_TEMP` | Double | °C | — | `C_to_F` | New signal needed in catalog |
| `BATTERY_VOLTAGE` | Double | V | `batteryVoltage` | None | 0–15.875V, with BatteryTag |
| `IGNITION_STATUS` | Enum (IgnitionStatus) | — | `ignitionOn` | `value_map` | Off→false, Accessory→false, On→true |
| `GEAR_LEVER_POSITION` | Enum (GearPosition) | — | `gearPosition` | `value_map` | PARK→0, REVERSE→1, NEUTRAL→2, DRIVE→3 |
| `TIRE_PRESSURE` | Double | kPa | `tire_fl/fr/rl/rr` | `kpa_to_psi` | Single signal with VEHICLE_WHEEL tag — connector must split into 4 messages |
| `SEAT_BELT_STATUS` | Enum (SeatbeltStatus) | — | `seatbeltStatus` | `value_map` | Buckled→true, Unbuckled→false. Has POSITION_ROW/SEAT tags |
| `ACCELERATION` | ThreeAxisSensor (x: lateral, y: longitudinal) | m/s² | `lateralG`, `acceleration` | `mps2_to_g` | Connector must split x/y into separate fields |
| `ACCELERATOR_PEDAL_POSITION` | Double | % | `throttle` | None | 0–100% |
| `OIL_LIFE_REMAINING` | Double | % | `oil_life` | None | |
| `TOTAL_ENGINE_TIME` | Double | s | `engine_hours_total` | seconds→hours | |
| `TRACTION_CONTROL_STATUS` | Enum (OffOnStatus) | — | `traction_control` | `value_map` | Off→false, On→true |
| `XEV_BATTERY_STATE_OF_CHARGE` | Double | % | `soc` | None | EV only |

**Connector responsibilities for Ford (protobuf → JSON):**
- Decode `google.protobuf.Any` typed_data into flat JSON fields
- Split compound signals: `TIRE_PRESSURE` (one signal with wheel tag) → four separate JSON fields (`tire_fl`, `tire_fr`, `tire_rl`, `tire_rr`)
- Split compound signals: `ACCELERATION` (x + y) → `lateral_accel` + `longitudinal_accel`
- Extract nested values: `SPEED.speed`, `HEADING.heading`, `POSITION.location.threeDPoint.latitude`
- Preserve `shard_key` as vehicle identifier (format: `aui:asset:vehicle/{uuid}`)
- Preserve `timestamp` from FeedEvent

**JSON output from connector** (what lands on `cms-telemetry-oem`):
```json
{
  "oem_source": "ford-feed",
  "shard_key": "aui:asset:vehicle/ec64c899-897c-4e88-801e-12b7995ed05d",
  "timestamp": "2026-01-15T12:30:00Z",
  "signal": "SPEED",
  "speed": 27.5,
  "speed_detection_type": "Speed_Wheel_Ticks"
}
```

The transform manifest then maps `speed` → `speed` (with `mps_to_mph`), `shard_key` → vehicleId (with `substring_after_last_slash`).

### 5.2 FordConnect REST API (Alternative)

Ford also offers a REST API (`FordConnect Query API`) that returns vehicle state as a single JSON response. This fits the generic REST polling pattern but is less suitable for continuous telemetry:
...
```
DynamoDB: cms-{stage}-fleet-enrollment
  PK: FLEET#{fleetId}
...
1. **OEMTelemetryProcessor** — Output clean JSON to `cms-telemetry-preprocessed` instead of gzip+base64 to `cms-telemetry-raw`. Fix Ford manifest signal names to match signal catalog `json_field` values.
...
3. **Transform Manifest S3 Loading** — Implement `loadManifestFromS3` in OEMTelemetryProcessor to replace hardcoded Ford manifest.
...
5. **Fleet Enrollment Table** ✅ COMPLETE
   - Created `cms-{stage}-fleet-enrollment` DynamoDB table
...
### Phase 4: Ford End-to-End

Ford connector deploys via `connector_stack.py` (ECS Fargate).

12. **Ford Connector Service** — ECS Fargate task running Ford Feed Service gRPC client. Authenticates, consumes shards, decodes protobuf, writes JSON to `cms-telemetry-oem`.
    - CDK: `deployment/stacks/connector_stack.py` (parameterized for gRPC streaming)
    - Makefile: `make deploy-connector-ford` (new target)

13. **Ford Transform Manifest** — Create validated `ford-transform.json` covering all available Ford Pro Telematics signals mapped to signal catalog.
    - S3: uploaded via `make seed-manifest-ford` (new target)

14. **OEM Integration Wizard Enhancement** — Pre-populate Ford connection pattern. Validate signal mappings against catalog in real-time.
...
|-----------|---------|----------|
| `mps_to_mph` | `value × 2.23694` | Ford speed (m/s → mph) |
...
| `kpa_to_psi` | `value × 0.145038` | Tire pressure (kPa → PSI) — Ford |
...
### 11.2 Ford Pro Telematics → CMS Signal Catalog

Ford provides data via FordConnect REST API (OAuth 2.0, polled). Key mappings:

Source: [Ford Developer Marketplace](https://www.developer.ford.com/apis), [Geotab Ford Data Set](https://support.geotab.com/oem-integration/doc/ford-data-set)

| Ford Field (wksSignal) | Source Path | CMS `json_field` | Unit Conversion | Notes |
|------------------------|-------------|-------------------|-----------------|-------|
...
Ford-specific notes:
- Ford uses `shardKey` format `aui:asset:vehicle/{uuid}` for vehicle identification. Extract UUID after last `/`.
- Message types are distinguished by `typedData.@type` field containing "Metric" or "Event".
- Ford Pro Telematics has Premium and Basic tiers — Basic lacks some signals (DTC codes, detailed engine data).
...
| CMS `json_field` | Direct/Sim | FWE (CAN) | Ford | Tesla | Notes |
|-------------------|:----------:|:---------:|:----:|:-----:|-------|
| `speed` | ✅ | ✅ | ✅ | ✅ | Universal |
| `odometer` | ✅ | ✅ | ✅ | ✅ | Universal |
| `lat` | ✅ | ✅ | ✅ | ✅ | Universal |
...
After reviewing cloud-to-cloud telemetry APIs across major providers (Geotab, Samsara, Smartcar, Ford, Tesla, GM, BMW, Mercedes, Volvo), we identified three integration patterns. Only two are generic enough for the wizard.
...
|---------|-------------------|------|----------|
| **REST Polling** | Loop: authenticate → poll endpoint → sleep → repeat | OAuth2 / API Key | Geotab (GetFeed), Samsara (stats/feed), Smartcar, FordConnect REST |
| **gRPC Streaming** | Connect → stream events with checkpointing → reconnect on drop | OAuth2 + checkpoints | Ford Pro Telematics Feed Service |
...
### 14.2 Streaming Connectors (Ford gRPC, Tesla WebSocket)

Both Ford and Tesla use persistent streaming connections for production fleet telemetry. The key difference is direction:

| | Ford gRPC | Tesla WebSocket |
|---|---|---|
...
3. **Multi-message aggregation**: Ford sends one signal per API response. Tesla sends all subscribed fields per message. The OEM processor currently handles single-signal messages. For Tesla, we may want to emit one CMS message with all fields rather than N separate messages.
